...
from synapseclient.models import Dataset, DatasetCollection, Project, Column, ColumnType
...
DATASET_IDS = ["syn65987017", "syn65987019", "syn65987020"] # Replace with your dataset IDs
test_dataset_collection = DatasetCollection(parent_id=project_id, name="test_dataset_collection").store()
...
print(len(my_retrieved_dataset_collection.items))
...
modified_data = pd.DataFrame(
    {"id": DATASET_IDS, "my_annotation": ["good dataset", "better dataset", "best dataset"]}
)
my_retrieved_dataset_collection.update_rows(values=modified_data, primary_keys=["id"], dry_run=False)
