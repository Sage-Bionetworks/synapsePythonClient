...
def test_command_line_store_and_submit():
    ...
    ## Create a project
    output = run('synapse --skip-checks store --name "%s" --description "test of store command" --type Project' % str(str(uuid.uuid4())))
    project_id = parse(r'Created entity:\s+(syn\d+)\s+', output)
    schedule_for_cleanup(project_id)
    ...
    filename = utils.make_bogus_data_file()
    schedule_for_cleanup(filename)
    output = run('synapse --skip-checks store --name "BogusFileEntity" --description "Bogus data to test file upload" --parentid %s --file %s' % (project_id, filename))
    file_entity_id = parse(r'Created entity:\s+(syn\d+)\s+', output)
    ...
    ## Submit a bogus file
    output = run('synapse --skip-checks submit --evaluation %s --name Some random name --teamName "My Team" --entity %s' %(eval.id, file_entity_id))
    ...
    filename = utils.make_bogus_data_file()
    schedule_for_cleanup(filename)
    output = run('synapse --skip-checks store --id %s --file %s' % (file_entity_id, filename,))
    updated_entity_id = parse(r'Updated entity:\s+(syn\d+)', output)
    

    ## Submit an updated bogus file
    output = run('synapse --skip-checks submit --evaluation %s --entity %s' %(eval.id, file_entity_id))
    submission_id = parse(r'Submitted \(id: (\d+)\) entity:\s+', output)
    ...
    output = run('synapse --skip-checks store --name Rubber Ducky --description I like rubber duckies --parentid %s --file %s' % (project_id, ducky_url))
    exteral_entity_id = parse(r'Created entity:\s+(syn\d+)\s+', output)
