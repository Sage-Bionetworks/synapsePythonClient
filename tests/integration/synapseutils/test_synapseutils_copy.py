...
def test_copy(syn, schedule_for_cleanup):
    ...
    pytest.raises(
        ValueError, synapseutils.copy, syn, schema.id, destinationId=second_project.id
    )
...
class TestCopyWiki:
    ...
    def test_copy_Wiki(self):
        ...
        for index, info in enumerate(second_headers):
            ...
            if info.get("parentId", None) is not None:
                ...
                assert (
                    info["parentId"] == mapping[self.first_headers[index]["parentId"]]
                )
        ...
        for index, info in enumerate(second_headers):
            ...
            orig_wikiPage = self.syn.getWiki(
                self.project_entity, self.first_headers[index]["id"]
            )
            ...
            orig_file = [i["fileName"] for i in orig_attach if not i["isPreview"]]
            new_file = [i["fileName"] for i in new_attach if not i["isPreview"]]
    ...
    def test_entitySubPageId_and_destinationSubPageId(self):
        ...
        test_ent_subpage = self.syn.getWiki(
            self.second_project.id, second_header[0]["id"]
        )
        ...
        third_header = synapseutils.copyWiki(
            self.syn,
            self.project_entity.id,
            self.second_project.id,
...
class TestCopyFileHandles:
    ...
    def init(self, syn, schedule_for_cleanup):
        ...
        external_response_1 = self.syn.restPOST(
            "/externalFileHandle",
            body=json.dumps(external_file_handle_request_1),
