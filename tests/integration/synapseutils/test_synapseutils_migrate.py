...
async def test_migrate_project(
    ...
    file_0 = synapseclient.File(
        name=file_0_name, path=file_0_path, parent=project_entity
    )
    ...
    file_1 = synapseclient.File(
        name=file_1_name, path=file_1_path, parent=folder_1_entity
    )
    ...
    file_2 = synapseclient.File(
        name=file_2_name, path=file_2_path, parent=folder_1_entity
    )
    ...
    file_3 = synapseclient.File(
        name=file_3_name, path=file_3_path, parent=folder_1_entity
    )
    ...
    table_1_entity = syn.store(
        synapseclient.RowSet(schema=table_1, rows=[synapseclient.Row(r) for r in data])
    )
    ...
    migration_result = synapseutils.migrate_indexed_files(syn, db_path, force=True)
    ...
    results = syn.tableQuery(
        "select file_col_1, file_col_2 from {}".format(utils.id_of(table_1_entity))
    )
    table_file_handles = []
    for row in results:
        for file_handle_id in row[2:]:
            file_handle = syn._getFileHandleDownload(
                file_handle_id, table_1_id, objectType="TableEntity"
            )["fileHandle"]
    ...
    assert (
        "id,type,version,row_id,col_name,from_storage_location_id,from_file_handle_id,to_file_handle_id,status,exception"
        in csv_lines
    )  # noqa
    assert (
        f"{file_0_entity.id},file,,,,{default_storage_location_id},{file_0_entity.dataFileHandleId},{file_0_entity_updated.dataFileHandleId},MIGRATED,"
        in csv_lines
    )  # noqa
    assert (
        f"{file_1_entity.id},file,,,,{default_storage_location_id},{file_1_entity.dataFileHandleId},{file_1_entity_updated.dataFileHandleId},MIGRATED,"
        in csv_lines
    )  # noqa
    assert (
        f"{file_2_entity.id},file,,,,{default_storage_location_id},{file_2_entity.dataFileHandleId},{file_2_entity_updated.dataFileHandleId},MIGRATED,"
        in csv_lines
    )  # noqa
    assert (
        f"{file_3_entity.id},file,,,,{default_storage_location_id},{file_3_entity.dataFileHandleId},{file_3_entity_updated.dataFileHandleId},MIGRATED,"
        in csv_lines
    )  # noqa
    assert (
        f"{table_1_id},table,1,1,file_col_1,{default_storage_location_id},{table_1_file_handle_1['id']},{table_file_handles[0]['id']},MIGRATED,"
        in csv_lines
    )  # noqa
    assert (
        f"{table_1_id},table,1,1,file_col_2,{default_storage_location_id},{table_1_file_handle_2['id']},{table_file_handles[1]['id']},MIGRATED,"
        in csv_lines
    )  # noqa
    assert (
        f"{table_1_id},table,1,2,file_col_1,{default_storage_location_id},{table_1_file_handle_3['id']},{table_file_handles[2]['id']},MIGRATED,"
        in csv_lines
    )  # noqa
    assert (
        f"{table_1_id},table,1,2,file_col_2,{default_storage_location_id},{table_1_file_handle_4['id']},{table_file_handles[3]['id']},MIGRATED,"
        in csv_lines
    )  # noqa
