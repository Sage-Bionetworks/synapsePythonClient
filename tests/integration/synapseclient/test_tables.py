...
def test_create_and_update_file_view(syn, project, schedule_for_cleanup):
    ...
    my_added_cols = [
        syn.store(Column(name=k, columnType="STRING")) for k in file_annotations.keys()
    ]
    my_added_cols_ids = [c["id"] for c in my_added_cols]
    view_default_ids = [
        c["id"]
        for c in syn._get_default_view_columns("entityview", EntityViewType.FILE.value)
    ]
    ...
    new_view_dict = list(
        csv.DictReader(io.open(new_view_results.filepath, encoding="utf-8", newline=""))
    )
...
def test_entity_view_add_annotation_columns(syn, project, schedule_for_cleanup):
    ...
    folder2 = syn.store(
        Folder(
            name=str(uuid.uuid4()) + "test_entity_view_add_annotation_columns_proj2",
            parent=project,
...
def test_rowset_tables(syn, project):
    ...
    row_reference_set1 = syn.store(RowSet(schema=schema1, rows=[Row(r) for r in data1]))
...
def test_store_table_datetime(syn, project):
    ...
    query_result = syn.tableQuery(
        "select * from %s" % utils.id_of(schema), resultsAs="rowset"
    )
...
def partial_rowset_test_state(syn, project):
    ...
    syn.store(
        File("~/path/doesnt/matter/again", name="f2", parent=folder, synapseStore=False)
    )
    ...
    view_schema = syn.store(
        EntityViewSchema(
            name="PartialRowTestViews" + str(uuid.uuid4()),
            columns=cols,
            addDefaultViewColumns=False,
            parent=project,
            scopes=[folder],
    ...
    expected_table_cells = pd.DataFrame(
        {"foo": [4.0, float("NaN")], "bar": [float("NaN"), 5.0]}
    )
    expected_view_cells = pd.DataFrame(
        {"foo": [float("NaN"), 7.0], "bar": [6.0, float("NaN")]}
    )
    ...
    def _query_with_retry(
        ...
        while time.time() - start_time < timeout:
            ...
            if expected_frame is not None:
                ...
                try:
                    assert_frame_equal(
                        df2, expected_frame, check_like=True, check_dtype=False
                    )
