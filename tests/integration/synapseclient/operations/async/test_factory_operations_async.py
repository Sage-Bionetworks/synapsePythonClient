"""Integration tests for the synapseclient.models.factory_operations get_async function."""
...
class TestFactoryOperationsGetAsync:
    """Tests for the synapseclient.models.factory_operations.get_async method."""
    ...
    async def test_get_async_project_by_id(self, project_model: Project) -> None:
        """Test retrieving a Project entity by Synapse ID."""
        # GIVEN a project exists
        project_id = project_model.id

        # WHEN I retrieve the project using get_async
        retrieved_project = await get_async(
            synapse_id=project_id, synapse_client=self.syn
        )
    ...
    async def test_get_async_project_by_name(self, project_model: Project) -> None:
        """Test retrieving a Project entity by name."""
        # GIVEN a project exists
        project_name = project_model.name

        # WHEN I retrieve the project using get_async with entity_name
        retrieved_project = await get_async(
    ...
    async def test_get_async_folder_by_id(self, project_model: Project) -> None:
        ...
        stored_folder = await folder.store_async(synapse_client=self.syn)
        self.schedule_for_cleanup(stored_folder.id)

        # WHEN I retrieve the folder using get_async
        retrieved_folder = await get_async(
            synapse_id=stored_folder.id, synapse_client=self.syn
        )
    ...
    async def test_get_async_folder_by_name(self, project_model: Project) -> None:
        ...
        # WHEN I retrieve the folder using get_async with entity_name
        retrieved_folder = await get_async(
    ...
    async def test_get_async_file_by_id_default_options(
        ...
        stored_file = await file.store_async(synapse_client=self.syn)
        self.schedule_for_cleanup(stored_file.id)

        # WHEN I retrieve the file using get_async with default options
        retrieved_file = await get_async(
            synapse_id=stored_file.id, synapse_client=self.syn
        )
    ...
    async def test_get_async_file_by_id_with_file_options(
        self, project_model: Project
    ) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # WHEN I retrieve the file using get_async with custom options
            retrieved_file = await get_async(
    ...
    async def test_get_async_file_by_id_metadata_only(
        self, project_model: Project
    ) -> None:
        ...
        # WHEN I retrieve the file using get_async without downloading
        retrieved_file = await get_async(
            synapse_id=stored_file.id,
            file_options=file_options,
            synapse_client=self.syn,
    ...
    async def test_get_async_file_by_id_with_activity(
        self, project_model: Project
    ) -> None:
        ...
        # WHEN I retrieve the file using get_async with activity options
        retrieved_file = await get_async(
            synapse_id=stored_file.id,
    ...
    async def test_get_async_file_by_id_specific_version(
        self, project_model: Project
    ) -> None:
    ...
    async def test_get_async_table_by_id_default_options(
        self, project_model: Project
    ) -> None:
        ...
        stored_table = await table.store_async(synapse_client=self.syn)
        self.schedule_for_cleanup(stored_table.id)

        # WHEN I retrieve the table using get_async
        retrieved_table = await get_async(
            synapse_id=stored_table.id, synapse_client=self.syn
        )
    ...
    async def test_get_async_table_by_id_with_table_options(
        self, project_model: Project
    ) -> None:
        ...
        # WHEN I retrieve the table using get_async without columns
        retrieved_table = await get_async(
    ...
    async def test_get_async_table_by_id_with_activity(
        self, project_model: Project
    ) -> None:
        ...
        # WHEN I retrieve the table using get_async with activity options
        retrieved_table = await get_async(
            synapse_id=stored_table.id,
            activity_options=activity_options,
            synapse_client=self.syn,
    ...
    async def test_get_async_dataset_by_id(self, project_model: Project) -> None:
        ...
        stored_dataset = await dataset.store_async(synapse_client=self.syn)
        self.schedule_for_cleanup(stored_dataset.id)

        # WHEN I retrieve the dataset using get_async
        retrieved_dataset = await get_async(
            synapse_id=stored_dataset.id, synapse_client=self.syn
        )
    ...
    async def test_get_async_dataset_collection_by_id(
        self, project_model: Project
    ) -> None:
        ...
        stored_collection = await dataset_collection.store_async(
            synapse_client=self.syn
        )
        self.schedule_for_cleanup(stored_collection.id)

        # WHEN I retrieve the dataset collection using get_async
        retrieved_collection = await get_async(
    ...
    async def test_get_async_entity_view_by_id(self, project_model: Project) -> None:
        ...
        stored_view = await entity_view.store_async(synapse_client=self.syn)
        self.schedule_for_cleanup(stored_view.id)

        # WHEN I retrieve the entity view using get_async
        retrieved_view = await get_async(
            synapse_id=stored_view.id, synapse_client=self.syn
        )
    ...
    async def test_get_async_submission_view_by_id(
        self, project_model: Project
    ) -> None:
        ...
        stored_view = await submission_view.store_async(synapse_client=self.syn)
        self.schedule_for_cleanup(stored_view.id)

        # WHEN I retrieve the submission view using get_async
        retrieved_view = await get_async(
            synapse_id=stored_view.id, synapse_client=self.syn
        )
    ...
    async def test_get_async_materialized_view_by_id(
        self, project_model: Project
    ) -> None:
        ...
        stored_source = await source_table.store_async(synapse_client=self.syn)
        ...
        stored_view = await materialized_view.store_async(synapse_client=self.syn)
        self.schedule_for_cleanup(stored_view.id)

        # WHEN I retrieve the materialized view using get_async
        retrieved_view = await get_async(
            synapse_id=stored_view.id, synapse_client=self.syn
        )
    ...
    async def test_get_async_virtual_table_by_id(self, project_model: Project) -> None:
        ...
        stored_virtual = await virtual_table.store_async(synapse_client=self.syn)
        self.schedule_for_cleanup(stored_virtual.id)

        # WHEN I retrieve the virtual table using get_async
        retrieved_virtual = await get_async(
            synapse_id=stored_virtual.id, synapse_client=self.syn
        )
    ...
    async def test_get_async_link_by_id_without_following(
        self, project_model: Project
    ) -> None:
        ...
        stored_link = await link.store_async(synapse_client=self.syn)
        ...
        # WHEN I retrieve the link using get_async without following
        retrieved_link = await get_async(
    ...
    async def test_get_async_link_by_id_default_follows_link(
        self, project_model: Project
    ) -> None:
        ...
        retrieved_entity = await get_async(
            synapse_id=stored_link.id,
            synapse_client=self.syn,
        )
    ...
    async def test_get_async_link_by_id_with_following(
        self, project_model: Project
    ) -> None:
        ...
        # WHEN I retrieve the link using get_async with default behavior (follow_link=True)
        retrieved_entity = await get_async(
            synapse_id=stored_link.id,
            synapse_client=self.syn,
        )
    ...
    async def test_get_async_link_by_id_with_following_explicit(
        self, project_model: Project
    ) -> None:
        ...
        # WHEN I retrieve the link using get_async with following
        retrieved_entity = await get_async(
            synapse_id=stored_link.id,
            link_options=link_options,
            synapse_client=self.syn,
    ...
    async def test_get_async_link_by_id_with_file_options(
        self, project_model: Project
    ) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # WHEN I retrieve the link using get_async with both link and file options
            retrieved_entity = await get_async(
                synapse_id=stored_link.id,
                link_options=link_options,
                file_options=file_options,
    ...
    async def test_get_async_with_entity_instance(self, project_model: Project) -> None:
        """Test get_async when passing an entity instance directly."""
        ...
        # WHEN I pass the entity instance to get_async with new options
        refreshed_file = await get_async(
    ...
    async def test_get_async_combined_options(self, project_model: Project) -> None:
        """Test get_async with multiple option types combined."""
    ...
    async def test_get_async_invalid_synapse_id_raises_error(self) -> None:
        """Test that get_async raises appropriate error for invalid Synapse ID."""
        ...
        with pytest.raises(Exception):  # Could be SynapseNotFoundError or similar
            await get_async(synapse_id=invalid_id, synapse_client=self.syn)

    async def test_get_async_invalid_entity_name_raises_error(
        self, project_model: Project
    ) -> None:
        """Test that get_async raises appropriate error for invalid entity name."""
    ...
    async def test_get_async_validation_errors(self) -> None:
