...
def test_state(syn: Synapse, project: Project, schedule_for_cleanup):
    class State:
        def __init__(self):
            ...
            self.desc_filename = _create_temp_file_with_cleanup(
                schedule_for_cleanup, self.description_text
            )
            self.update_description_text = "'SOMEBODY ONCE TOLD ME THE WORLD WAS GONNA ROLL ME I AINT THE SHARPEST TOOL IN THE SHED'"
...
def test_command_line_client(test_state):
    ...
    output = run(
        test_state, "synapse", "mv", "--id", file_entity_id, "--parentid", folder.id
    )
...
def test_command_copy(test_state):
    ...
    externalURL_entity = test_state.syn.store(
    ...
    test_state.syn.set_annotations(
        Annotations(externalURL_entity, externalURL_entity.etag, annots)
    )
    ...
    assert (
        copied_ent.properties.dataFileHandleId
        == file_entity.properties.dataFileHandleId
    )
    ...
    assert (
        copied_URL_ent.properties.dataFileHandleId
        == externalURL_entity.properties.dataFileHandleId
    )
...
def test_command_line_using_paths(test_state):
    ...
    output = run(test_state, "synapse" "--skip-checks", "show", filename)
    ...
    run(test_state, "synapse" "--skip-checks", "associate", path, "-r")
...
def test_create__same_project_name(test_state):
    ...
    name = str(uuid.uuid4())
    output_first = run(test_state, "synapse", "create", "--name", name, "Project")

    entity_id_first = parse(r"Created entity:\s+(syn\d+)\s+", output_first)

    test_state.schedule_for_cleanup(entity_id_first)

    output_second = run(test_state, "synapse", "create", "--name", name, "Project")

    entity_id_second = parse(r"Created entity:\s+(syn\d+)\s+", output_second)
...
class TestSchemaManagementCommands:
    ...
    @pytest.fixture(scope="function")
    def schema_test_state(self, syn: Synapse, project: Project, schedule_for_cleanup):
        ...
        class SchemaState:
            def __init__(self):
                ...
                self.organization = SchemaOrganization(org_name)
                self.organization.store(synapse_client=syn)
                ...
                json.dump(schema_definition, self.schema_file)
        ...
        def cleanup():
            ...
            if os.path.exists(state.schema_file.name):
                os.remove(state.schema_file.name)
            # Clean up organization and its schemas
            for schema in state.organization.get_json_schemas(synapse_client=syn):
                schema.delete(synapse_client=syn)
            state.organization.delete(synapse_client=syn)

        schedule_for_cleanup(cleanup)
        return state

    def test_register_json_schema(self, schema_test_state):
        ...
        output = run(
            ...
            schema_test_state.schema_file.name,
            schema_test_state.organization.name,
            schema_name,
            "--schema-version",
            "1.0.0",
        )
        ...
        assert schema_test_state.organization.name in output

    def test_bind_json_schema(self, schema_test_state):
        ...
        schema_uri = f"{schema_test_state.organization.name}-{schema_name}-1.0.0"

        # Create a test folder
        folder = Folder(
            name=f"test.folder.{str(uuid.uuid4())[:8]}",
            parent_id=schema_test_state.project.id,
        )
        folder.store(synapse_client=schema_test_state.syn)
        ...
        try:
            ...
            # Cleanup folder
            schema_test_state.syn.delete(folder.id)

    def test_register_and_bind_workflow(self, schema_test_state):
        ...
        try:
            ...
            schema_uri = f"{schema_test_state.organization.name}-{schema_name}-2.0.0"
            output = run(
                schema_test_state,
                "synapse",
                "--skip-checks",
            ...
            retrieved_folder = schema_test_state.syn.get(folder.id, downloadFile=False)
            bound_schema = retrieved_folder.get_schema(
                synapse_client=schema_test_state.syn
            )
