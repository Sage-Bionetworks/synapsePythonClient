...
class TestDownloadCaching:
    ...
    async def test_redownload_after_local_rename_same_content(
        self,
        mocker: MockerFixture,
        syn: Synapse,
        project_model: Project,
        ...
        """
        Contrast with the upload-side equivalent, Cache.get() has no equivalent
        md5 fallback for downloads -- it only ever matches by recorded path, so
        renaming a cached file, even within the same directory, invalidates
        the stale cache entry and forces a real re-download.
        """
        spy_file_handle = mocker.spy(download_functions, "download_by_file_handle")
        ...
        # AND the file is in the cache under its original path
        assert syn.cache.contains(
            file_handle_id=file.file_handle.id, path=original_file_path
        )

        # WHEN the cached file is renamed, with no change to its content
        renamed_path = f"{original_file_path}.renamed"
        os.rename(original_file_path, renamed_path)
        schedule_for_cleanup(renamed_path)

        redownloaded_file = await File(
            id=file.id, path=os.path.dirname(renamed_path)
        ).get_async(synapse_client=syn)

        # THEN a real download occurs since the cache entry pointing to the old path (no longer exists) is removed
        spy_file_handle.assert_called_once()
        assert utils.equal_paths(original_file_path, redownloaded_file.path)
        assert utils.md5_for_file_hex(
            filename=redownloaded_file.path
        ) == utils.md5_for_file_hex(filename=renamed_path)

    async def test_redownload_after_moved_to_different_directory_same_content_no_explicit_destination(
        self,
        mocker: MockerFixture,
        syn: Synapse,
        project_model: Project,
        ...
        """
        File is moved to an entirely different directory instead of renamed in place,
        and no explicit download path is given.
        The cache can't be retrieved since the cached file is no longer accessible and can't determine if it is unmodified.
        So cache.get() returns None and a real download occurs.
        """
        spy_file_handle = mocker.spy(download_functions, "download_by_file_handle")
        ...
        # WHEN the cached file is moved to a different directory, with no change to its content
        other_dir = tempfile.mkdtemp()
        schedule_for_cleanup(other_dir)
        moved_path = shutil.move(
            original_file_path,
            os.path.join(other_dir, os.path.basename(original_file_path)),
        )

        # when no explicit destination is given, the file is downloaded to the synapse cache location
        redownloaded_file = await File(id=file.id).get_async(synapse_client=syn)

        # THEN a real download occurs.
        spy_file_handle.assert_called_once()
        assert utils.equal_paths(
            os.path.dirname(redownloaded_file.path), synapse_cache_location
        )
        assert utils.md5_for_file_hex(
            filename=redownloaded_file.path
        ) == utils.md5_for_file_hex(filename=moved_path)

    async def test_redownload_after_moved_to_different_directory_same_content_with_explicit_destination(
        self,
        mocker: MockerFixture,
        syn: Synapse,
        project_model: Project,
        ...
        spy_file_handle.assert_called_once()
        assert utils.equal_paths(
            os.path.dirname(redownloaded_file.path), os.path.dirname(original_file_path)
        )
        assert utils.md5_for_file_hex(
            filename=redownloaded_file.path
        ) == utils.md5_for_file_hex(filename=moved_path)

    async def test_redownload_after_casing_change_both_dir_file_on_case_sensitive_filesystem(
        self,
        mocker: MockerFixture,
        syn: Synapse,
        project_model: Project,
        ...
        """
        Test the behavior when both the containing directory and the filename
        are re-cased. Forcing os.path.normcase to compare case-sensitively
        makes Cache.get()'s directory-match branch miss, but the fallback
        loop still finds the entry unmodified, so no download occurs.
        """
        spy_file_handle = mocker.spy(download_functions, "download_by_file_handle")
        ...
        # WHEN both the containing directory and the filename are re-cased with no change to content
        recased_file_path = original_file_path.upper()
        os.rename(original_file_path, recased_file_path)
        schedule_for_cleanup(recased_file_path)

        with (
            patch("os.path.normcase", side_effect=lambda p: p),
            patch(
                "synapseclient.api.entity_factory.download_file_entity_model",
                wraps=spy_for_async_function(
                    download_functions.download_file_entity_model
                ),
            ) as spy_file_entity,
        ):
            redownloaded_file = await File(
                id=file.id, path=os.path.dirname(recased_file_path)
            ).get_async(synapse_client=syn)

        # THEN no download occurs since cache.get() returns the cached file path and the file is unmodified
        spy_file_entity.assert_called_once()
        spy_file_handle.assert_not_called()
        # AND the cached content is copied into the requested (recased) directory
        assert utils.equal_paths(
            os.path.dirname(redownloaded_file.path), os.path.dirname(recased_file_path)
        )
        assert utils.md5_for_file_hex(
            filename=redownloaded_file.path
        ) == utils.md5_for_file_hex(filename=recased_file_path)

    async def test_redownload_after_casing_change_only_file_on_case_sensitive_filesystem(
        self,
        mocker: MockerFixture,
        syn: Synapse,
        project_model: Project,
        ...
        """
        Test the behavior when only the filename's casing changes and the
        containing directory is untouched. Since the requested directory
        string is identical to the cached entry's directory, Cache.get()'s
        directory-match succeeds, and the resolved download_path lands back
        on the exact cached path so no download occurs. No download occurs.
        """
        spy_file_handle = mocker.spy(download_functions, "download_by_file_handle")
        ...
        recased_file_path = os.path.join(
            os.path.dirname(original_file_path),
            os.path.basename(original_file_path).upper(),
        )
        os.rename(original_file_path, recased_file_path)
        schedule_for_cleanup(recased_file_path)
        ...
        # THEN no download occurs since cache.get() returns the cached file path and the file is unmodified
        # and downloaded filename is extracted from the os.path.basename(cached_file_path)
        # so the redownloaded file path is the original file path.
        spy_file_entity.assert_called_once()
        spy_file_handle.assert_not_called()
        assert utils.equal_paths(redownloaded_file.path, original_file_path)
        assert utils.md5_for_file_hex(
            filename=redownloaded_file.path
        ) == utils.md5_for_file_hex(filename=recased_file_path)

    async def test_redownload_after_casing_change_both_dir_file_on_case_insensitive_filesystem(
        self,
        mocker: MockerFixture,
        syn: Synapse,
        project_model: Project,
        ...
        """
        Contrast with the case-sensitive variant: on a case-insensitive
        filesystem, Cache.get()'s directory-match compares paths via the
        real (unpatched) os.path.normcase, so a recased directory is still
        recognized as the same directory. No download occurs and  and the
        cached content is copied into the recased directory under a
        keep-both disambiguated name.
        """
        spy_file_handle = mocker.spy(download_functions, "download_by_file_handle")
        ...
        with (
            patch("os.path.normcase", side_effect=str.lower),
            patch(
                "synapseclient.api.entity_factory.download_file_entity_model",
                wraps=spy_for_async_function(
                    download_functions.download_file_entity_model
        ...
        spy_file_handle.assert_not_called()
        # AND the cached content is copied into the requested (recased) directory --
        # under a disambiguated name, since the directory string matches the cached entry's directory but file name is different
        # so a collision resolution is triggered.
        assert utils.equal_paths(
            os.path.dirname(redownloaded_file.path), os.path.dirname(recased_file_path)
        )
        assert utils.md5_for_file_hex(
    ...
    async def test_redownload_after_casing_change_only_file_on_case_insensitive_filesystem(
        self,
        mocker: MockerFixture,
        syn: Synapse,
        project_model: Project,
        ...
        """
        Contrast with the case-sensitive variant: with normcase patched to
        str.lower instead of forced to identity, the directory-match still
        succeeds so no download occurs.
        """
        spy_file_handle = mocker.spy(download_functions, "download_by_file_handle")
        ...
        # THEN no download occurs since cache.get() returns the cached file path as the file is unmodified
        # and downloaded filename is extracted from the file handle so the redownloaded file path is the original file path.
        spy_file_entity.assert_called_once()
        spy_file_handle.assert_not_called()
        assert utils.equal_paths(redownloaded_file.path, original_file_path)
        assert utils.md5_for_file_hex(
    ...
    async def test_redownload_after_casing_change_only_dir_on_case_sensitive_filesystem(
        self,
        mocker: MockerFixture,
        syn: Synapse,
        project_model: Project,
        ...
        """
        Test the behavior when only the containing directory's casing
        changes and the filename itself is untouched. Forcing os.path.normcase
        to compare case-sensitively makes the directory-match miss, but the
        fallback loop still finds the entry unmodified -- so this behaves the
        same as the both-dir-and-file variant: no download occurs, and the
        cached content is copied into the recased directory under a
        keep-both disambiguated name.
        """
        spy_file_handle = mocker.spy(download_functions, "download_by_file_handle")
        ...
        # WHEN only the containing directory is re-cased, with no change to the filename or content
        recased_file_path = os.path.join(
            os.path.dirname(original_file_path).upper(),
        ...
        spy_file_handle.assert_not_called()
        # AND the cached content is copied into the requested (recased) directory --
        # under a disambiguated name, since the directory string matches the cached
        # entry's directory but file name is different so a collision resolution is triggered.
        assert utils.equal_paths(
            os.path.dirname(redownloaded_file.path), os.path.dirname(recased_file_path)
        )
        assert utils.md5_for_file_hex(
    ...
    async def test_redownload_after_casing_change_only_dir_on_case_insensitive_filesystem(
        self,
        mocker: MockerFixture,
        syn: Synapse,
        project_model: Project,
        ...
        """
        Contrast with the case-sensitive variant: with str.lower-folded
        normcase, the directory-match succeeds directly (rather than via the
        fallback loop), but the resulting download_path lands back on the exact
        cached path so no download occurs. No download occurs.
        """
        spy_file_handle = mocker.spy(download_functions, "download_by_file_handle")
