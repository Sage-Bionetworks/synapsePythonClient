...
class TestRecordSetStoreAsync:
    """Tests for the RecordSet.store_async method."""
    ...
    async def test_store_in_project_async(
        self, project_model: Project, record_set_fixture: RecordSet
    ) -> None:
        # GIVEN a RecordSet
        record_set_fixture.name = str(uuid.uuid4())

        # WHEN I store the RecordSet in a project asynchronously
        stored_record_set = await record_set_fixture.store_async(
    ...
    async def test_store_in_folder_async(
        self, project_model: Project, record_set_fixture: RecordSet
    ) -> None:
        ...
        # WHEN I store the RecordSet in the folder asynchronously
        stored_record_set = await record_set_fixture.store_async(
    ...
    async def test_store_with_activity_async(
        self, project_model: Project, record_set_fixture: RecordSet
    ) -> None:
        ...
        # WHEN I store the RecordSet asynchronously
        stored_record_set = await record_set_fixture.store_async(
            parent=project_model, synapse_client=self.syn
        )
        self.schedule_for_cleanup(stored_record_set.id)
    ...
    async def test_store_with_annotations_async(
        self, project_model: Project, record_set_fixture: RecordSet
    ) -> None:
    ...
    async def test_store_update_existing_record_set_async(
        self, project_model: Project, record_set_fixture: RecordSet
    ) -> None:
        # GIVEN an existing RecordSet
        record_set_fixture.name = str(uuid.uuid4())
        original_record_set = await record_set_fixture.store_async(
            parent=project_model, synapse_client=self.syn
        )
        self.schedule_for_cleanup(original_record_set.id)

        # WHEN I update the RecordSet with new metadata asynchronously
        original_record_set.description = "Updated description"
        original_record_set.version_comment = "Updated version comment"

        updated_record_set = await original_record_set.store_async(
...
class TestRecordSetGetAsync:
    """Tests for the RecordSet.get_async method."""
    ...
    async def stored_record_set(self, project_model: Project) -> RecordSet:
    ...
    async def test_get_record_set_by_id_async(
        self, stored_record_set: RecordSet
    ) -> None:
        # GIVEN an existing RecordSet
        original_id = stored_record_set.id

        # WHEN I get the RecordSet by ID asynchronously
        retrieved_record_set = await RecordSet(id=original_id).get_async(
            synapse_client=self.syn
        )
    ...
    async def test_get_record_set_with_activity_async(
        ...
        # WHEN I get the RecordSet with activity asynchronously
        retrieved_record_set = await RecordSet(id=record_set.id).get_async(
    ...
    async def test_get_non_existent_record_set_async(self) -> None:
...
class TestRecordSetDeleteAsync:
    """Tests for the RecordSet.delete_async method."""
    ...
    async def test_delete_entire_record_set_async(self, project_model: Project) -> None:
        # GIVEN an existing RecordSet
        filename = utils.make_bogus_uuid_file()
        self.schedule_for_cleanup(filename)
        ...
        # WHEN I delete the entire RecordSet asynchronously
        await record_set.delete_async(synapse_client=self.syn)

        # THEN the RecordSet should be deleted and no longer accessible
        with pytest.raises(SynapseHTTPError):
            await RecordSet(id=record_set_id).get_async(synapse_client=self.syn)

    async def test_delete_specific_version_async(self, project_model: Project) -> None:
        ...
        v2_record_set = await record_set.store_async(synapse_client=self.syn)

        # WHEN I delete only version 2 asynchronously
        await v2_record_set.delete_async(version_only=True, synapse_client=self.syn)
    ...
    async def test_delete_validation_errors_async(self) -> None:
        # GIVEN a RecordSet without an ID
        record_set = RecordSet()
        ...
        with pytest.raises(ValueError):
            await record_set_with_id.delete_async(
                version_only=True, synapse_client=self.syn
            )


class TestRecordSetGetDetailedValidationResultsAsync:
    """Tests for the RecordSet.get_detailed_validation_results_async method."""
    ...
    async def test_get_validation_results_no_file_handle_id_async(
        self, project_model: Project
    ) -> None:
    ...
    async def test_get_validation_results_with_default_location_async(
