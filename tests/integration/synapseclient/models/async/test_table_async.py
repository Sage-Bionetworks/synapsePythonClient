...
class TestTableCreation:
    ...
    async def test_create_table_with_different_column_configurations(
        ...
        # WHEN I store the table
        table_single_column = await table_single_column.store_async(
        ...
        # AND I can retrieve that table from Synapse
        new_table_instance = await Table(id=table_single_column.id).get_async(
        ...
        # WHEN I store the table
        table_multi_columns = await table_multi_columns.store_async(
            synapse_client=self.syn
        )
        self.schedule_for_cleanup(table_multi_columns.id)

        # THEN the table should be created and columns are correct
        new_table_instance = await Table(id=table_multi_columns.id).get_async(
            synapse_client=self.syn, include_columns=True
        )
        assert (
            new_table_instance.columns["test_column"].column_type == ColumnType.STRING
    ...
    async def test_table_creation_with_data_sources(
        ...
        table_dict = await table_dict.store_async(synapse_client=self.syn)
        self.schedule_for_cleanup(table_dict.id)
        await table_dict.store_rows_async(
            values=dict_data,
        ...
        # WHEN I store the table and then add data
        table_df = await table_df.store_async(synapse_client=self.syn)
        self.schedule_for_cleanup(table_df.id)
        await table_df.store_rows_async(
            values=df_data,
            schema_storage_strategy=SchemaStorageStrategy.INFER_FROM_DATA,
            synapse_client=self.syn,
        )
        ...
        table_csv = await table_csv.store_async(synapse_client=self.syn)
        self.schedule_for_cleanup(table_csv.id)
        await table_csv.store_rows_async(
            values=filepath,
            schema_storage_strategy=SchemaStorageStrategy.INFER_FROM_DATA,
            synapse_client=self.syn,
        )
...
class TestRowStorage:
    ...
    async def test_store_rows_with_json_containing_apostrophes(
        self, project_model: Project
    ) -> None:
