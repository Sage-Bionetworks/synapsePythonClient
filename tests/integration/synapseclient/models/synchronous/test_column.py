...
class TestColumn:
    ...
    async def test_get_column_by_id(self, project_model: Project) -> None:
        ...
        retrieved_table = Table(id=table.id).get(
        ...
        column = Column(id=column_id).get(synapse_client=self.syn)
    ...
    async def test_get_column_by_invalid_id(self) -> None:
        ...
        with pytest.raises(SynapseHTTPError) as exc_info:
            Column(id=invalid_id).get(synapse_client=self.syn)
    ...
    async def test_list_all_columns(self, project_model: Project) -> None:
        ...
        columns = []
        for column in Column.list(limit=5, synapse_client=self.syn):
    ...
    async def test_list_columns_with_prefix(self, project_model: Project) -> None:
        ...
        # Note: limit is per request, the function continues until all matching columns are returned
        columns = []
        for column in Column.list(prefix=prefix, limit=5, synapse_client=self.syn):
            columns.append(column)
    ...
    async def test_list_columns_with_limit_and_offset(
        ...
        columns_page1 = []
        for column in Column.list(limit=3, offset=0, synapse_client=self.syn):
        ...
        columns_page2 = []
        for column in Column.list(limit=3, offset=3, synapse_client=self.syn):
