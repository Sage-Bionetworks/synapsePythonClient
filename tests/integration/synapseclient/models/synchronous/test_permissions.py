...
class TestAcl:
    ...
    async def test_get_acl_default(
        ...
        permissions = entity.get_acl(principal_id=user.id)
    ...
    async def test_get_acl_through_team(
        ...
        entity.set_permissions(principal_id=user.id, access_type=[])
    ...
    async def test_get_acl_through_multiple_teams(
        ...
        entity.set_permissions(principal_id=team_2.id, access_type=team_2_permissions)
    ...
    async def test_get_acl_with_public_and_authenticated_users(
        ...
        entity.set_permissions(principal_id=PUBLIC, access_type=["READ"])
        ...
        public_permissions = entity.get_acl()
        ...
        user_permissions = entity.get_acl(principal_id=user.id)
    ...
    async def test_get_acl_for_subfolder_with_different_permissions(
        ...
        subfolder_permissions = subfolder.get_acl(principal_id=user.id)

        # AND getting permissions for the parent folder
        parent_permissions = parent_folder.get_acl(principal_id=user.id)
    ...
    async def test_remove_team_permissions_with_empty_access_type(
        ...
        team_acl_before = entity.get_acl(principal_id=team.id)
        ...
        user_acl = entity.get_acl(principal_id=user.id)
    ...
    async def test_table_permissions(self, project_model: Project) -> None:
        ...
        table.set_permissions(principal_id=team.id, access_type=[])

        await asyncio.sleep(random.randint(10, 20))

        # THEN team should no longer have permissions
        team_acl_after_delete = table.get_acl(principal_id=team.id)
        assert team_acl_after_delete == []

        # BUT other permissions should remain
        auth_acl_after = table.get_acl(principal_id=AUTHENTICATED_USERS)
    ...
    async def test_entity_view_permissions(self, project_model: Project) -> None:
        ...
        team_acl = entity_view.get_acl(principal_id=team.id)
        assert set(team_permissions) == set(team_acl)

        # Verify authenticated users permissions
        auth_acl = entity_view.get_acl(principal_id=AUTHENTICATED_USERS)
        assert set(["READ", "DOWNLOAD"]) == set(auth_acl)
        ...
        all_acls = entity_view.list_acl()
        assert isinstance(all_acls, AclListResult)
        assert len(all_acls.entity_acl) >= 3  # User, team, authenticated_users

        # WHEN deleting authenticated users permissions
        entity_view.set_permissions(principal_id=AUTHENTICATED_USERS, access_type=[])

        await asyncio.sleep(random.randint(10, 20))

        # THEN authenticated users should lose permissions
        auth_acl_after = entity_view.get_acl(principal_id=AUTHENTICATED_USERS)
        assert auth_acl_after == []

        # AND user permissions should no longer include DOWNLOAD
        user_acl_after = entity_view.get_acl(principal_id=user.id)
        assert set(limited_user_permissions + ["MODERATE"]) == set(user_acl_after)

        # BUT team permissions should remain
        team_acl_after = entity_view.get_acl(principal_id=team.id)
    ...
    async def test_submission_view_permissions(self, project_model: Project) -> None:
        ...
        team1_acl = submission_view.get_acl(principal_id=team1.id)
        assert set(team1_permissions) == set(team1_acl)

        team2_acl = submission_view.get_acl(principal_id=team2.id)
        assert set(team2_permissions) == set(team2_acl)

        # Check public permissions
        public_acl = submission_view.get_acl()
        assert set(["READ"]) == set(public_acl)

        # Check user effective permissions (should aggregate from all teams)
        user_effective_acl = submission_view.get_acl(principal_id=user.id)
        ...
        submission_view.set_permissions(principal_id=PUBLIC, access_type=[])
        submission_view.set_permissions(principal_id=team1.id, access_type=[])

        # THEN PUBLIC should lose all permissions
        public_acl_after = submission_view.get_acl(principal_id=PUBLIC)
        assert public_acl_after == []

        # AND team should lose all permissions
        team_acl_after = submission_view.get_acl(principal_id=team1.id)
        assert team_acl_after == []

        # AND user effective permissions should no longer include team1 permissions
        user_effective_after = submission_view.get_acl(principal_id=user.id)
        ...
        team2_acl_after = submission_view.get_acl(principal_id=team2.id)
...
class TestPermissionsForCaller:
    ...
    async def test_get_permissions_with_limited_access(self) -> None:
        ...
        project.set_permissions(principal_id=user.id, access_type=["READ"])
    ...
    async def test_get_permissions_through_teams(self) -> None:
        ...
        # AND the user has no direct permissions
        project.set_permissions(principal_id=user.id, access_type=[])
...
class TestDeletePermissions:
    ...
    async def test_delete_permissions_simple_tree_structure(
        ...
        await self._set_custom_permissions(folder_a)
        await self._set_custom_permissions(file_1)
        await asyncio.sleep(random.randint(10, 20))
        ...
        await self._verify_permissions_deleted(folder_a)
        await self._verify_permissions_deleted(file_1)
    ...
    async def test_delete_permissions_deep_nested_structure(
        ...
        for entity in structure.values():
            await self._set_custom_permissions(entity)
        await asyncio.sleep(random.randint(10, 20))
        ...
        # THEN all permissions should be deleted
        for entity in structure.values():
            await self._verify_permissions_deleted(entity)
    ...
    async def test_delete_permissions_folder_with_only_files(
        ...
        # Set permissions on all entities
        await self._set_custom_permissions(folder)
        await self._set_custom_permissions(file)
        await asyncio.sleep(random.randint(10, 20))
        ...
        # THEN all permissions should be deleted
        await self._verify_permissions_deleted(folder)
        await self._verify_permissions_deleted(file)
    ...
    async def test_delete_permissions_target_files_only_complex(
        ...
        # Set permissions on all entities
        await self._set_custom_permissions(structure["shallow_folder"])
        await self._set_custom_permissions(structure["shallow_file"])
        await self._set_custom_permissions(structure["deep_branch"])
        await self._set_custom_permissions(structure["sub_deep"])
        ...
        await self._verify_permissions_deleted(structure["shallow_file"])
        for file in structure["deep_files"]:
            await self._verify_permissions_deleted(file)
    ...
    async def test_delete_permissions_include_container_only_deep_structure(
        ...
        await self._verify_permissions_deleted(structure["level_1"])
        await self._verify_permissions_deleted(structure["file_at_1"])
        await self._verify_permissions_deleted(structure["level_2"])

        # BUT deeper nested entities should retain permissions
        await self._verify_permissions_not_deleted(structure["level_3"])
        await self._verify_permissions_not_deleted(structure["level_4"])
        await self._verify_permissions_not_deleted(structure["file_at_2"])
        await self._verify_permissions_not_deleted(structure["file_at_3"])
        await self._verify_permissions_not_deleted(structure["file_at_4"])
    ...
    async def test_delete_permissions_skip_self_complex_structure(
        ...
        # Set permissions on all entities
        await self._set_custom_permissions(structure["mixed_folder"])
    ...
    async def test_delete_permissions_dry_run_no_changes(
        ...
        await self._verify_permissions_not_deleted(folder_a)
        await self._verify_permissions_not_deleted(file_1)
    ...
    async def test_delete_permissions_selective_branches(
        ...
        # Set permissions on all entities
        await self._set_custom_permissions(branch_a)
        await self._set_custom_permissions(branch_b)
        await self._set_custom_permissions(file_a)
        await self._set_custom_permissions(file_b)
        await asyncio.sleep(random.randint(10, 20))
        ...
        await self._verify_permissions_deleted(branch_a)
        await self._verify_permissions_deleted(file_a)

        # BUT branch_b should retain permissions
        await self._verify_permissions_not_deleted(branch_b)
        await self._verify_permissions_not_deleted(file_b)
    ...
    async def test_delete_permissions_no_container_content_but_has_children(
        ...
        await self._set_custom_permissions(parent_folder)
        await self._set_custom_permissions(child_file)
        await asyncio.sleep(random.randint(10, 20))
...
class TestAllEntityTypesPermissions:
    ...
    async def test_list_acl_all_entity_types(self) -> None:
    ...
    async def test_list_acl_specific_entity_types(self) -> None:
    ...
    async def test_delete_permissions_all_entity_types(self) -> None:
    ...
    async def test_delete_permissions_all_entity_types_actual_deletion(self) -> None:
    ...
    async def test_mixed_case_entity_types_actual_deletion(self) -> None:
