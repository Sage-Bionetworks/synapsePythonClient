"""Integration tests for the synapseclient.models.CurationTask class."""
...
class TestFileBasedMetadataTaskProperties:
    ...
    async def test_fill_from_dict_and_to_synapse_request(self) -> None:
        ...
        response_dict = {
            ...
            "uploadFolderId": "syn123456",
            "fileViewId": "syn789012",
        }

        # WHEN I create a FileBasedMetadataTaskProperties from the dict
        properties = FileBasedMetadataTaskProperties().fill_from_dict(response_dict)

        # THEN the properties should be correctly populated
        assert properties.upload_folder_id == "syn123456"
        assert properties.file_view_id == "syn789012"

        # AND WHEN I convert it back to a request dict
        request_dict = properties.to_synapse_request()

        # THEN the request dict should be correctly formatted
    ...
    async def test_to_synapse_request_with_partial_data(self) -> None:
        ...
        # THEN only the non-None fields should be included
        expected_concrete_type = (
            "org.sagebionetworks.repo.model.curation.metadata."
            "FileBasedMetadataTaskProperties"
        )
        assert request_dict["concreteType"] == expected_concrete_type
        assert request_dict["uploadFolderId"] == "syn123456"
        assert "fileViewId" not in request_dict
...
class TestRecordBasedMetadataTaskProperties:
    ...
    async def test_fill_from_dict_and_to_synapse_request(self) -> None:
        ...
        # WHEN I create a RecordBasedMetadataTaskProperties from the dict
        properties = RecordBasedMetadataTaskProperties().fill_from_dict(response_dict)

        # THEN the properties should be correctly populated
        assert properties.record_set_id == "syn123456"
    ...
    async def test_to_synapse_request_with_none_values(self) -> None:
        ...
        # THEN only the concrete type should be included
        expected_concrete_type = (
            "org.sagebionetworks.repo.model.curation.metadata."
            "RecordBasedMetadataTaskProperties"
        )
        assert request_dict["concreteType"] == expected_concrete_type
        assert "recordSetId" not in request_dict


class TestCurationTaskStore:
    """Tests for the CurationTask.store method."""
    ...
    @pytest.fixture(scope="function")
    def record_set(self, project_model: Project) -> RecordSet:
    ...
    async def test_store_file_based_curation_task(
        ...
        stored_task = curation_task.store(synapse_client=self.syn)
    ...
    async def test_store_update_existing_curation_task(
        ...
        stored_updated_task = updated_task.store(synapse_client=self.syn)
    ...
    async def test_store_validation_errors(self) -> None:
        ...
        with pytest.raises(ValueError, match="project_id is required"):
            curation_task.store(synapse_client=self.syn)
...
class TestCurationTaskGet:
    """Tests for the CurationTask.get method."""
    ...
    async def test_get_curation_task(
        ...
        # WHEN I get the task by ID
        retrieved_task = CurationTask(task_id=original_task.task_id).get(
...
class TestCurationTaskDelete:
    """Tests for the CurationTask.delete method."""
    ...
    async def test_delete_curation_task(
        ...
        curation_task.delete(synapse_client=self.syn)
...
class TestCurationTaskList:
    """Tests for the CurationTask.list method."""
    ...
    async def test_list_curation_tasks(
        ...
        # WHEN I list all curation tasks for the project
        listed_tasks = list(
            CurationTask.list(project_id=project_model.id, synapse_client=self.syn)
        )
    ...
    async def test_list_empty_project(self) -> None:
        ...
        # WHEN I list curation tasks for the project
        listed_tasks = list(
            CurationTask.list(project_id=empty_project.id, synapse_client=self.syn)
        )
