...
class test_copy_permissions:
    ...
    def setup(self):
        ...
        self.schema_ent = Schema(name='Testing', parent=self.project_entity.id)
        self.schema_ent.id = "syn2456"
        self.user_name = {'userName':'test'}

    def test_copy_file_read_permissions(self):
        #TEST: File entity READ permissions not copied
        permissions = ["READ"]
        access_requirements = {'totalNumberOfResults':0}

        with patch.object(syn, "get", return_value=self.file_ent) as patch_syn_get, \
             patch.object(syn, "getUserProfile", return_value=self.user_name) as patch_syn_get_profile, \
    ...
    def test_copy_schema_read_permissions(self):
        #TEST: Schema entity READ permissions not copied
        permissions = ["READ"]
        access_requirements = {'totalNumberOfResults':0}
        with patch.object(syn, "get", return_value=self.schema_ent) as patch_syn_get, \
             patch.object(syn, "getUserProfile", return_value=self.user_name) as patch_syn_get_profile, \
             patch.object(syn, "getPermissions", return_value=permissions) as patch_syn_permissions, \
             patch.object(syn, "restGET", return_value=access_requirements) as patch_syn_restget:
            copied_file = synapseutils.copy(syn, patch_syn_get.id, destinationId=self.second_project.id, skipCopyWikiPage=True)
