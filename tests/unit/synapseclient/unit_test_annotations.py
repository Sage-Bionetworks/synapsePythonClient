...
def test_to_synapse_annotations__called_convert_to_annotations_list(
    mock_convert_to_annotations_list,
):
    ...
    mock_convert_to_annotations_list.assert_called_once_with(
        {"foo": "bar", "zoo": ["zing", "zaboo"], "species": "Platypus"}
    )
...
def test_to_synapse_annotations():
    ...
    expected = {
        ...
        "annotations": {
            ...
            "zoo": {"value": ["zing", "zaboo"], "type": "STRING"},
...
def test_annotations_unicode():
    a = Annotations(
        ...
        {
            "files": ["文件.txt"],
            "cacheDir": "/Users/chris/.synapseCache/python/syn1809087",
            "foo": 1266,
        },
    )
    sa = to_synapse_annotations(a)
    expected = {
        ...
        "annotations": {
            ...
            "files": {"type": "STRING", "value": ["文件.txt"]},
            "foo": {"type": "LONG", "value": ["1266"]},
        },
    }
    assert expected == sa
...
def test_submission_status_annotations_round_trip():
    ...
    pytest.raises(
        KeyError, set_privacy, sa, key="this_key_does_not_exist", is_private=False
    )
...
def test_submission_status_double_annos():
    ssa = {
        "longAnnos": [{"isPrivate": False, "value": 13, "key": "lucky"}],
        "doubleAnnos": [
            {"isPrivate": False, "value": 3, "key": "three"},
...
def test_to_synapse_annotations__empty():
    python_client_annos = Annotations(
...
def test_is_synapse_annotation():
    ...
    assert not is_synapse_annotations(
        {"etag": "0f2977b9-0261-4811-a89e-c13e37ce4604", "annotations": {}}
    )
    # missing etag
    assert not is_synapse_annotations({"id": "syn123", "annotations": {}})
    # missing annotations
    assert not is_synapse_annotations(
        {"id": "syn123", "etag": "0f2977b9-0261-4811-a89e-c13e37ce4604"}
    )
    # has additional keys
    assert is_synapse_annotations(
        {
...
class TestAnnotations:
    def test__None_id_and_etag(self):
        ...
        pytest.raises(
            ValueError, Annotations, None, "0f2977b9-0261-4811-a89e-c13e37ce4604"
        )

        # after constuction
        annot = Annotations(
            "syn123", "0f2977b9-0261-4811-a89e-c13e37ce4604", {"asdf": "qwerty"}
        )
...
def test_check_annotations_changed():
    ...
    mock_bundle_annotations = {
        "annotations": {
            "far": {"type": "LONG", "value": ["123"]},
    ...
    mock_bundle_annotations = {
        "annotations": {
            "far": {"type": "DOUBLE", "value": ["123.12", "456.33"]},
