...
def test_activity_creation_from_dict():
    ...
    d = {
        ...
        "used": [
            {
                "reference": {"targetId": "syn12345", "versionNumber": 42},
                "wasExecuted": True,
            }
        ],
    }
...
def test_activity_creation_by_constructor():
    ...
    ue1 = {
        "reference": {"targetId": "syn101", "targetVersionNumber": 42},
        "wasExecuted": False,
    }
    ue2 = {
        "id": "syn102",
...
def test_activity_used_url():
    ...
    a.executed(
        url="http://cran.r-project.org/web/packages/glmnet/index.html", name="glm.net"
    )
    a.used(
    ...
    u = utils._find_used(
        a, lambda res: "name" in res and res["name"] == "Seriously advanced code"
    )
    assert u is not None
    assert u["url"] == u3["url"]
...
def test_activity_parameter_errors():
    ...
    pytest.raises(
        SynapseMalformedEntityError,
        a.used,
        ["syn12345", "http://google.com"],
        url="http://amazon.com",
    )
    pytest.raises(
        SynapseMalformedEntityError, a.used, "syn12345", url="http://amazon.com"
    )
    pytest.raises(
        SynapseMalformedEntityError, a.used, "http://amazon.com", targetVersion=1
    )
