...
class TestUploadAttempt:
    ...
    def test_fetch_presigned_urls(self, syn):
        ...
        expected_uri = "/file/multipart/{upload_id}/presigned/url/batch".format(
            upload_id=upload_id
        )
        expected_body = {
            "uploadId": upload_id,
    ...
    def test_refresh_presigned_part_url__fetch_required(self, syn):
        ...
        with mock.patch.object(
            ...
            fetch_urls.assert_called_once_with(
                upload._upload_id, list(original_presigned_urls.keys())
            )
    ...
    def test_handle_part__500(self, syn):
        ...
        self._handle_part_success_test(
            ...
            [
                (mock.call(pre_signed_url, chunk, headers=signed_headers), mock_500),
                (
    ...
    def test_handle_part_success(self, syn):
        ...
        upload._pre_signed_part_urls = {
            part_number: (pre_signed_url_1, signed_headers_1)
        }
    ...
    def test_call_upload(self, syn):
        ...
        with mock.patch.object(
            ...
            upload_response = {"state": "COMPLETED"}
            ...
            restPUT.assert_called_once_with(
                "/file/multipart/{upload_id}/complete".format(upload_id=upload_id),
                requests_session=mock_session,
                endpoint=upload._syn.fileHandleEndpoint,
            )

    def _test_call_upload__part_exception(
        self, syn, part_exception, expected_raised_exception
    ):
        upload = self._init_upload_attempt(syn)
    ...
    def test_all_parts_completed(self, syn):
        ...
        with mock.patch.object(
            ...
            assert (
                f"/file/multipart/{upload_id}/complete" in restPUT.call_args_list[0][0]
            )
...
class TestMultipartUpload:
    ...
    def test_multipart_upload_file(self):
        ...
        with mock.patch("os.path.exists") as os_path_exists, mock.patch(
            ...
            with pytest.raises(IOError):
                multipart_upload_file(
                    syn, file_path, storage_location_id=storage_location_id
                )
            ...
            multipart_upload_file(
                syn, file_path, storage_location_id=storage_location_id, **kwargs
            )
            mock_multipart_upload.assert_called_once_with(
                syn,
    ...
    def test_multipart_upload_string(self):
        ...
        with mock.patch.object(
            ...
            # call w/ default args
            multipart_upload_string(
                syn, upload_text, storage_location_id=storage_location_id
            )
    ...
    def test_multipart_copy__default_args(self):
        ...
        with mock.patch.object(
            multipart_upload, "_multipart_upload"
        ) as mock_multipart_upload:
            expected_upload_request = {
    ...
    def _multipart_upload_test(self, upload_side_effect, syn, *args, **kwargs):
        with mock.patch.object(
            multipart_upload, "UploadAttempt"
