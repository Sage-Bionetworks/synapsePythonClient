...
def test_parent_id_required():
    ...
    pytest.raises(SynapseMalformedEntityError, File, 'http://xkcd.com/1343/', name='XKCD: Manuals', synapseStore=False)
...
def test_split_entity_namespaces():
    ...
    assert set(properties.keys()) == {'concreteType', 'name', 'parentId', 'dataFileHandleId'}
    assert properties['name'] == 'Henry'
...
def test_DockerRepository__no_repositoryName():
    with pytest.raises(SynapseMalformedEntityError):
...
def test_is_versionable_non_entity():
    pytest.raises(ValueError, is_versionable, dict())


def test_is_versionable_dict_representation_of_entity():
    assert not is_versionable({
        "id": "syn123456",
        "concreteType": "org.sagebionetworks.repo.model.Folder",
        "parentId": "syn445566",
        "name": "Testing123"
