...
def test_cast_values() -> None:
    selectColumns = [
        ...
        {"id": "355", "name": "x", "columnType": "DOUBLE"},
        {"id": "356", "name": "n", "columnType": "INTEGER"},
        {"id": "357", "name": "bonk", "columnType": "BOOLEAN"},
        {"id": "358", "name": "boom", "columnType": "LINK"},
    ]
    ...
    # group by
    selectColumns = [
        {"name": "bonk", "columnType": "BOOLEAN"},
        {"name": "COUNT(name)", "columnType": "INTEGER"},
        {"name": "AVG(x)", "columnType": "DOUBLE"},
        {"name": "SUM(n)", "columnType": "INTEGER"},
    ]
...
def test_cast_values__list_type() -> None:
    selectColumns = [
        ...
        {"id": "357", "name": "bonk", "columnType": "BOOLEAN_LIST"},
        {"id": "357", "name": "bar", "columnType": "ENTITYID_LIST"},
        {"id": "357", "name": "bam", "columnType": "USERID_LIST"},
        {"id": "358", "name": "boom", "columnType": "DATE_LIST"},
    ]
...
def test_csv_to_pandas_df_no_kwargs():
    ...
    with (
        ...
        df = _csv_to_pandas_df(
            ...
            escape_char=synapseclient.table.DEFAULT_ESCAPE_CHAR,
        ...
        mock_read_csv.assert_called_once_with(
            ...
            escapechar=synapseclient.table.DEFAULT_ESCAPE_CHAR,
            header=0,
            skiprows=0,
            lineterminator=None,
        )
...
def test_row_set_table() -> None:
    row_set_json = {
        ...
        "rows": [
            {"rowId": 5, "values": ["foo", "1.23", "2.2", "101"], "versionNumber": 3},
            {"rowId": 6, "values": ["bar", "1.34", "2.4", "101"], "versionNumber": 3},
            {"rowId": 7, "values": ["foo", "1.23", "2.2", "101"], "versionNumber": 4},
            {"rowId": 8, "values": ["qux", "1.23", "2.2", "102"], "versionNumber": 3},
        ],
        "tableId": "syn2976298",
    }
...
def test_pandas_to_table() -> None:
    ...
    # A dataframe with no row id and version
    df = pd.DataFrame(
        index=["1_7", "2_7", "3_8"], data=dict(a=[100, 200, 300], b=["c", "d", "e"])
    )
    ...
    test_df = pd.DataFrame(
        {
            ...
            "datetime2": ("2017-07-06 00:00:00", "2017-07-06 00:00:00", None),
        }
    )
    schema = Schema(name="Foo", parent="syn12345")
...
def test_csv_table() -> None:
    ...
    try:
        ...
        with io.open(filename, mode="w", encoding="utf-8", newline="") as temp:
            writer = csv.writer(
                temp, quoting=csv.QUOTE_NONNUMERIC, lineterminator=str(os.linesep)
            )
...
def test_list_of_rows_table() -> None:
    ...
    schema1 = Schema(
        name="Jazz Guys", columns=cols, id="syn1000002", parent="syn1000001"
    )

    # need columns to do cast_values w/o storing
    table = Table(
        schema1, data, headers=[SelectColumn.from_column(col) for col in cols]
    )
...
def test_aggregate_query_result_to_data_frame() -> None:
    class MockSynapse(object):
        def _queryTable(
            ...
            return {
                ...
                "selectColumns": [
                    {"columnType": "STRING", "id": "1387", "name": "State"}
                ],
            }
    ...
    assert list(df["MIN(Born)"].values) == [
        1935,
        1928,
        1929,
        1926,
...
def test_wait_for_async() -> None:
    ...
    # return a mocked https://rest-docs.synapse.org/rest/org/sagebionetworks/repo/model/asynch/AsynchronousJobStatus.html
    syn.restGET = MagicMock(
        return_value={
            "jobState": "PROCESSING",
    ...
    pytest.raises(
        SynapseTimeoutError, syn._waitForAsync, uri="foo/bar", request={"foo": "bar"}
    )
...
def test_downloadTableColumns(syn: Synapse, download_location: str) -> None:
    ...
    table = MagicMock(
        tableId="abc",
        headers=[header],
        __iter__=MagicMock(return_value=iter([[1], [2], [3]])),
    )
    ...
    cached_paths = [None, "/tmp/foo", None]
...
def test_build_table_download_file_handle_list__repeated_file_handles(
    ...
    ) = syn._build_table_download_file_handle_list(table, ["filehandle"], None)
...
def test_SubmissionViewSchema__before_synapse_store(syn: Synapse) -> None:
    with (
        ...
        mocked_get_default.assert_called_once_with(
            "submissionview", view_type_mask=None
        )
        mocked_get_annotations.assert_called_once_with(
            ["123"], "submissionview", view_type_mask=None
        )
...
def test_EntityViewSchema__before_synapse_store(syn: Synapse) -> None:
    ...
    with (
        ...
        submission_view._before_synapse_store(syn)
        mocked_get_default.assert_called_once_with("entityview", view_type_mask=1)
        mocked_get_annotations.assert_called_once_with(
...
def test_EntityViewSchema__ignore_column_names_set_info_preserved() -> None:
    ...
    ignored_names = {"a", "b", "c"}
    entity_view = EntityViewSchema(
        "someName", parent="syn123", ignoredAnnotationColumnNames={"a", "b", "c"}
    )
...
def test_EntityViewSchema__ignore_annotation_column_names(syn: Synapse) -> None:
    ...
    with (
        ...
        mocked_get_annotations.assert_called_once_with(
            scopeIds, "entityview", view_type_mask=EntityViewType.FILE.value
        )
...
class TestTableQueryResult:
    ...
    def setup_method(self) -> None:
        ...
        self.query_result_dict = {
            "queryResult": {
                "queryResults": {
                    "headers": [{"columnType": "STRING", "name": "col_name"}],
                    "rows": self.rows,
                    "tableId": "syn123",
                }
            },
            "selectColumns": [
                {"columnType": "STRING", "id": "1337", "name": "col_name"}
            ],
        }
...
class TestPartialRow:
    ...
    def test_constructor__values_translation(self) -> None:
        ...
        expected_values = [
            {"key": "12345", "value": "rowValue"},
            {"key": "09876", "value": "otherValue"},
        ]
...
class TestCsvFileTable:
    ...
    def test_as_data_frame__boolean_like_string_column(self) -> None:
        ...
        schema1 = Schema(
            id="syn1234", name="Jazz Guys", columns=cols, parent="syn1000001"
        )
...
def test_get_view_type_mask_for_deprecated_type() -> None:
    ...
    assert EntityViewType.PROJECT.value == _get_view_type_mask_for_deprecated_type(
        "project"
    )
    assert (
