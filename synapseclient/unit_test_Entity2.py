...
import collections
from entity2 import Entity, Project, Folder, File, Data
...
def test_Entity():
    ...
    ## should take the value of the property rather than the annotation
...
def test_property_generator():
    ...
    for key,value in project.properties():
        if key=='name':
            ...
            assert value=='syn1001'
        elif key=='foo':
            assert False

    for key,value in project.annotations():
        if key=='foo':
            assert value=='bar'
        else:
            assert False
...
def test_subclassing():
    ...
    class FoobarEntity(Entity):
        def __init__(self, x, **kwargs):
            super(FoobarEntity, self).__init__(entityType='org.sagebionetworks.repo.model.Foobar', **kwargs)
            self.x = x

    foobar = FoobarEntity(123, name='Foobar', foo=123)
    assert foobar.x == 123
