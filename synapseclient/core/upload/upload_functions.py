...
import synapseclient.core.remote_file_storage_wrappers
...
def upload_file_handle(syn, parent_entity, path, synapseStore=True, md5=None, file_size=None, mimetype=None):
    ...
    entity_parent_id = synapseclient.core.utils.id_of(parent_entity)
    ...
    # synapse managed S3
    if upload_destination_type == synapseclient.core.constants.SYNAPSE_S3_UPLOAD_DESTINATION \
            or upload_destination_type == synapseclient.core.constants.EXTERNAL_S3_UPLOAD_DESTINATION:
    ...
    elif upload_destination_type == synapseclient.core.constants.EXTERNAL_UPLOAD_DESTINATION:
        if location['uploadType'] == 'SFTP':
            syn.logger.info('\n%s\n%s\nUploading to: %s\n%s\n' % ('#' * 50, location.get('banner', ''),
                                                                  urllib.parse.urlparse(location['url']).netloc,
                                                                  '#' * 50))
    ...
    elif upload_destination_type == synapseclient.core.constants.EXTERNAL_OBJECT_STORE_UPLOAD_DESTINATION:
...
def create_external_file_handle(syn, path, mimetype=None, md5=None, file_size=None):
    ...
    url = synapseclient.core.utils.as_url(os.path.expandvars(os.path.expanduser(path)))
    if synapseclient.core.utils.is_url(url):
        parsed_url = urllib.parse.urlparse(url)
        if parsed_url.scheme == 'file' and os.path.isfile(parsed_url.path):
            actual_md5 = synapseclient.core.utils.md5_for_file(parsed_url.path).hexdigest()
            if md5 is not None and md5 != actual_md5:
                raise synapseclient.core.models.SynapseMd5MismatchError(
    ...
    if is_local_file:
        syn.cache.add(file_handle['id'], synapseclient.core.utils.file_url_to_path(url))
    return file_handle
...
def upload_external_file_handle_sftp(syn, file_path, sftp_url, mimetype=None):
    ...
    uploaded_url = synapseclient.core.remote_file_storage_wrappers.SFTPWrapper.upload_file(
        file_path, urllib.parse.unquote(sftp_url), username, password)

    file_handle = syn._createExternalFileHandle(uploaded_url, mimetype=mimetype,
                                                md5=synapseclient.core.utils.md5_for_file(file_path).hexdigest(),
...
def upload_synapse_s3(syn, file_path, storageLocationId=None, mimetype=None):
    file_handle_id = synapseclient.core.upload.multipart_upload_file(syn, file_path, contentType=mimetype,
...
def upload_client_auth_s3(syn, file_path, bucket, endpoint_url, key_prefix, storage_location_id, mimetype=None):
    ...
    synapseclient.core.remote_file_storage_wrappers.S3ClientWrapper.upload_file(
