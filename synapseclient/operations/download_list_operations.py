"""Operations for the user's Synapse Download List (cart).
...
Files are not packaged into a zip because download lists can exceed 100 GB.
Instead, files are downloaded individually and removed from the list after
successful download, so interrupted runs are safely resumable.
"""
...
@dataclass
class _DownloadListManifestRequest(AsynchronousCommunicator):
    ...
    **Phase 4 — Download** (_post_exchange_async)
    ...
    def fill_from_dict(
        ...
        Returns:
            The _DownloadListManifestRequest object instance.
        """
    ...
    *,
    ...
    if parallel:
        ...
        async def bounded_download(
            ...
            async with sem:
                return await _download_manifest_file(
                    row,
                    download_location=download_location,
                    synapse_client=synapse_client,
                )
        ...
        for row in rows:
            item = await _download_manifest_file(
                row,
                download_location=download_location,
                synapse_client=synapse_client,
            )
