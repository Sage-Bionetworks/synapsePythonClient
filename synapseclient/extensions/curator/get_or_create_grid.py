...
def get_or_create_curator_grid(
    ...
    *,
    ...
    return wrap_async_to_sync(
        coroutine=get_or_create_curator_grid_async(
            task_id=task_id,
...
async def get_or_create_curator_grid_async(
    task_id: int,
    *,
    owner_principal_id: int | None = None,
    timeout: int = 120,
    ...
    Example: Get or create the grid for an assigned curation task
        ...
        import asyncio
        import synapseclient
        from synapseclient.extensions.curator import get_or_create_curator_grid_async

        syn = synapseclient.Synapse()
        syn.login()

        grid = asyncio.run(get_or_create_curator_grid_async(task_id=123))
        print(f"Grid session: {grid.session_id}")
        ```
    ...
    task = await CurationTask(task_id=task_id).get_async(synapse_client=client)

    # Step 2: check whether a grid session is already attached to the task.
    status = await task.get_status_async(synapse_client=client)
    ...
    if active_session_id:
        try:
            return await Grid(session_id=active_session_id).get_async(
    ...
    return await task.create_grid_session_async(
        owner_principal_id=owner_principal_id,
        timeout=timeout,
        synapse_client=client,
    )
