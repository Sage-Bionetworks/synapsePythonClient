...
class Synapse(object):
    ...
    def __init__(self, repoEndpoint=None, authEndpoint=None, fileHandleEndpoint=None, portalEndpoint=None,
        ...
        cache_root_dir = synapseclient.core.cache.CACHE_ROOT_DIR
        ...
        self.cache = synapseclient.core.cache.Cache(cache_root_dir)
        ...
        synapseclient.core.credentials.cached_sessions.migrate_old_session_file_credentials_if_necessary(self)
    ...
    def debug(self, value):
        ...
        logger_name = synapseclient.core.logging_setup.DEBUG_LOGGER_NAME if value \
            else synapseclient.core.logging_setup.DEFAULT_LOGGER_NAME
    ...
    def login(self, email=None, password=None, apiKey=None, sessionToken=None, rememberMe=False, silent=False,
        ...
        **Linux** systems.
        ...
        if not self.skip_checks:
            synapseclient.core.version_check.version_check()

        # Make sure to invalidate the existing session
        self.logout()

        credential_provder_chain = synapseclient.core.credentials.get_default_credential_chain()
        ...
        self.credentials = credential_provder_chain.get_credentials(
            self,
            synapseclient.core.credentials.UserLoginArgs(email, password, apiKey, forced, sessionToken))

        # Final check on login success
        if not self.credentials:
            raise synapseclient.core.models.SynapseNoCredentialsError("No credentials provided.")

        # Save the API key in the cache
        if rememberMe:
            synapseclient.core.credentials.cached_sessions.set_api_key(self.credentials.username, self.credentials.api_key)
            synapseclient.core.credentials.cached_sessions.set_most_recent_user(self.credentials.username)
    ...
    def _get_config_authentication(self):
        return self._get_config_section_dict(synapseclient.core.constants.AUTHENTICATION_SECTION_NAME)
    ...
    def _getSessionToken(self, email, password):
        ...
        except synapseclient.core.models.SynapseHTTPError as err:
            if err.response.status_code == 403 or err.response.status_code == 404 or err.response.status_code == 401:
                raise synapseclient.core.models.SynapseAuthenticationError("Invalid username or password.")
            raise
    ...
    def logout(self, forgetMe=False):
        ...
        if forgetMe:
            synapseclient.core.credentials.cached_sessions.remove_api_key(self.credentials.username)
    ...
    def getUserProfile(self, id=None, sessionToken=None, refresh=False):
        ...
        try:
            ...
            if isinstance(id, collections.Mapping) and 'ownerId' in id:
                id = id.ownerId
            elif isinstance(id, synapseclient.TeamMember):
        ...
        return synapseclient.UserProfile(**self.restGET(uri, headers={'sessionToken': sessionToken} if sessionToken else None))
    ...
    def _findPrincipals(self, query_string):
        ...
        """
        uri = '/userGroupHeaders?prefix=%s' % urllib.parse.quote(query_string)
        return [synapseclient.UserGroupHeader(**result) for result in self._GET_paginated(uri)]
    ...
    def onweb(self, entity, subpageId=None):
        ...
        synId = synapseclient.core.utils.id_of(entity)
    ...
    def printEntity(self, entity, ensure_ascii=True):
        ...
        if synapseclient.core.utils.is_synapse_id(entity):
    ...
    def get(self, entity, **kwargs):
        ...
        elif isinstance(entity, str) and not synapseclient.core.utils.is_synapse_id(entity):
            raise synapseclient.core.models.SynapseFileNotFoundError(('The parameter %s is neither a local file path '
                                            ' or a valid entity id' % entity))
        # have not been saved entities
        elif isinstance(entity, synapseclient.Entity) and not entity.get('id'):
    ...
    def _check_entity_restrictions(self, restrictionInformation, entity, downloadFile):
        if restrictionInformation['hasUnmetAccessRequirement']:
            warning_message = ("\nThis entity has access restrictions. Please visit the web page for this entity "
                               ...
                               % synapseclient.core.utils.id_of(entity))
            if downloadFile:
                raise synapseclient.core.models.SynapseUnmetAccessRestrictions(warning_message)
    ...
    def _getFromFile(self, filepath, limitSearch=None):
        ...
        """
        results = self.restGET('/entity/md5/%s' % synapseclient.core.utils.md5_for_file(filepath).hexdigest())['results']
        if limitSearch is not None:
            ...
            results = [ent for ent, path in zip(results, paths) if
                       synapseclient.core.utils.is_in_path(limitSearch, path)]
        if len(results) == 0:  # None found
            raise synapseclient.core.models.SynapseFileNotFoundError('File %s not found in Synapse' % (filepath,))
        elif len(results) > 1:
    ...
    def move(self, entity, new_parent):
        ...
        entity = self.get(entity, downloadFile=False)
        entity.parentId = synapseclient.core.utils.id_of(new_parent)
    ...
    def _getWithEntityBundle(self, entityBundle, entity=None, **kwargs):
        ...
        local_state = entity.local_state() if entity and isinstance(entity, synapseclient.Entity) else {}
        if path is not None:
            local_state['path'] = path
        properties = entityBundle['entity']
        annotations = synapseclient.annotations.from_synapse_annotations(entityBundle['annotations'])
        entity = synapseclient.Entity.create(properties, annotations, local_state)
    ...
    def _resolve_download_path_collisions(self, downloadLocation, file_name, ifcollision, synapseCache_location,
        ...
        if synapseclient.core.utils.normalize_path(downloadLocation) == synapseCache_location:
        ...
        downloadPath = synapseclient.core.utils.normalize_path(os.path.join(downloadLocation, file_name))
        # resolve collison
        if os.path.exists(downloadPath):
            if ifcollision == "overwrite.local":
                ...
                if downloadPath != cached_file_path:
                    return synapseclient.core.utils.unique_filename(downloadPath)
            else:
    ...
    def store(self, obj, **kwargs):
        ...
        if not (isinstance(obj, synapseclient.Entity) or type(obj) == dict):
            if isinstance(obj, synapseclient.Wiki):
        ...
        entity = obj
        properties, annotations, local_state = synapseclient.entity.split_entity_namespaces(entity)
        bundle = None
        # Anything with a path is treated as a cache-able item
        if entity.get('path', False):
            if 'concreteType' not in properties:
                properties['concreteType'] = synapseclient.File._synapse_entity_type
            ...
            if needs_upload:
                ...
                fileHandle = synapseclient.core.upload.upload_file_handle(
                    self,
                    entity['parentId'],
        ...
        if 'id' in properties:
            ...
            try:
            ...
            except synapseclient.core.models.SynapseHTTPError as ex:
                if createOrUpdate and ex.response.status_code == 409:
                    ...
                    existing_annos = synapseclient.annotations.from_synapse_annotations(bundle['annotations'])
        ...
        if used or executed:
            if activity is not None:
                raise synapseclient.core.models.SynapseProvenanceError(
                    'Provenance can be specified as an Activity object or as used/executed item(s), but not both.')
            activityName = kwargs.get('activityName', None)
            activityDescription = kwargs.get('activityDescription', None)
            activity = synapseclient.Activity(name=activityName, description=activityDescription, used=used, executed=executed)
    ...
    def _createAccessRequirementIfNone(self, entity):
        ...
        """
        existingRestrictions = self.restGET('/entity/%s/accessRequirement?offset=0&limit=1'
                                            % synapseclient.core.utils.id_of(entity))
        if len(existingRestrictions['results']) <= 0:
            self.restPOST('/entity/%s/lockAccessRequirement' % synapseclient.core.utils.id_of(entity), body="")
    ...
    def _getEntityBundle(self, entity, version=None, bitFlags=0x800 | 0x40000 | 0x2 | 0x1):
        ...
        if version is not None:
            uri = '/entity/%s/version/%d/bundle?mask=%d' \
                  % (synapseclient.core.utils.id_of(entity), int(version), bitFlags)
        else:
            uri = '/entity/%s/bundle?mask=%d' % (synapseclient.core.utils.id_of(entity), bitFlags)
    ...
    def delete(self, obj, version=None):
        ...
        if isinstance(obj, str):
            if version:
                self.restDELETE(uri='/entity/%s/version/%s' % (synapseclient.core.utils.id_of(obj), version))
            else:
                self.restDELETE(uri='/entity/%s' % synapseclient.core.utils.id_of(obj))
            ...
            try:
                ...
                synapseclient.core.models.SynapseError("Can't delete a %s" % type(obj))
    ...
    def _get_user_name(self, user_id):
        if user_id not in self._user_name_cache:
            self._user_name_cache[user_id] = synapseclient.core.utils.extract_user_name(self.getUserProfile(user_id))
    ...
    def _list(self, parent, recursive=False, long_format=False, show_modified=False, indent=0, out=sys.stdout):
        ...
        for result in results:
            ...
            fmt_fields = {'name': result['name'],
                          'id': result['id'],
                          'padding': ' ' * indent,
                          'slash_or_not': '/' if synapseclient.entity.is_container(result) else ''}
            fmt_string = "{id}"

            if long_format:
                fmt_fields['createdOn'] = synapseclient.core.utils.iso_to_datetime(result['createdOn'])\
                    .strftime("%Y-%m-%d %H:%M")
            ...
            if show_modified:
                fmt_fields['modifiedOn'] = synapseclient.core.utils.iso_to_datetime(result['modifiedOn'])\
                    .strftime("%Y-%m-%d %H:%M")
            ...
            if (indent == 0 or recursive) and synapseclient.entity.is_container(result):
        ...
        if indent == 0 and not results_found:
            out.write('No results visible to {username} found for id {id}\n'
                      .format(username=self.credentials.username, id=synapseclient.core.utils.id_of(parent)))
    ...
    def uploadFileHandle(self, path, parent, synapseStore=True, mimetype=None, md5=None, file_size=None):
        ...
        """
        return synapseclient.core.upload.upload_file_handle(self, parent, path, synapseStore, md5, file_size, mimetype)
    ...
    def uploadFile(self, entity, filename=None, used=None, executed=None):
        ...
        if 'name' not in properties or properties['name'] is None:
            properties['name'] = synapseclient.core.utils.guess_file_name(filename)

        return self.store(synapseclient.File(properties=properties, annotations=annotations, local_state=local_state), used=used,
                          executed=executed)
    ...
    def uploadSynapseManagedFileHandle(self, path, storageLocationId=None, mimetype=None):
        ...
        """
        return synapseclient.core.upload.upload_synapse_s3(self, path, storageLocationId=storageLocationId, mimetype=mimetype)
    ...
    def _uploadToFileHandleService(self, filename, synapseStore=True, mimetype=None, md5=None, fileSize=None,
        ...
        elif synapseclient.core.utils.is_url(filename):
            if synapseStore and urllib.parse.urlparse(filename).scheme != 'sftp':
        ...
        # For local files, we default to uploading the file unless explicitly instructed otherwise
            ...
            if synapseStore:
                file_handle_id = synapseclient.core.upload.multipart_upload(self, filename, contentType=mimetype,
    ...
    def _getRawAnnotations(self, entity, version=None):
        ...
        if version:
            uri = '/entity/%s/version/%s/annotations' % (synapseclient.core.utils.id_of(entity), str(version))
        else:
            uri = '/entity/%s/annotations' % synapseclient.core.utils.id_of(entity)
    ...
    def getAnnotations(self, entity, version=None):
        ...
        """
        return synapseclient.annotations.from_synapse_annotations(self._getRawAnnotations(entity, version))
    ...
    def setAnnotations(self, entity, annotations={}, **kwargs):
        ...
        synapseAnnos = synapseclient.annotations.to_synapse_annotations(annotations)
        synapseAnnos['id'] = synapseclient.core.utils.id_of(entity)
        ...
        return synapseclient.annotations.from_synapse_annotations(self.restPUT(uri, body=json.dumps(synapseAnnos)))
    ...
    def getChildren(self, parent, includeTypes=["folder", "file", "table", "link", "entityview", "dockerrepo"],
        ...
        """
        parentId = synapseclient.core.utils.id_of(parent) if parent is not None else None
    ...
    def _getBenefactor(self, entity):
        ...
        if synapseclient.core.utils.is_synapse_id(entity) or synapseclient.entity.is_synapse_entity(entity):
            return self.restGET('/entity/%s/benefactor' % synapseclient.core.utils.id_of(entity))
        return entity
    ...
    def _storeACL(self, entity, acl):
        ...
        if hasattr(entity, 'putACLURI'):
            ...
            entity_id = synapseclient.core.utils.id_of(entity)
    ...
    def setPermissions(self, entity, principalId=None, accessType=['READ', 'DOWNLOAD'], modify_benefactor=False,
        ...
        benefactor = self._getBenefactor(entity)
        if benefactor['id'] != synapseclient.core.utils.id_of(entity):
            if modify_benefactor:
                ...
                self.logger.warning('Creating an ACL for entity %s, which formerly inherited access control from a'
                                    ...
                                    % (synapseclient.core.utils.id_of(entity), benefactor['name'], benefactor['id']))
    ...
    def getProvenance(self, entity, version=None):
        ...
        if version:
            uri = '/entity/%s/version/%d/generatedBy' % (synapseclient.core.utils.id_of(entity), version)
        else:
            uri = '/entity/%s/generatedBy' % synapseclient.core.utils.id_of(entity)
        return synapseclient.Activity(data=self.restGET(uri))
    ...
    def setProvenance(self, entity, activity):
        ...
        if 'id' in activity:
            ...
            activity = synapseclient.Activity(data=self.restPUT(uri, json.dumps(activity)))
        else:
            activity = self.restPOST('/activity', body=json.dumps(activity))

        # assert that an entity is generated by an activity
        uri = '/entity/%s/generatedBy?generatedBy=%s' % (synapseclient.core.utils.id_of(entity), activity['id'])
        activity = synapseclient.Activity(data=self.restPUT(uri))
    ...
    def updateActivity(self, activity):
        ...
        uri = '/activity/%s' % activity['id']
        return synapseclient.Activity(data=self.restPUT(uri, json.dumps(activity)))
    ...
    def _getFileHandleDownload(self, fileHandleId,  objectId, objectType='FileEntity'):
        ...
        if failure == 'NOT_FOUND':
            raise synapseclient.core.models.SynapseFileNotFoundError("The fileHandleId %s could not be found" % fileHandleId)
        elif failure == "UNAUTHORIZED":
            raise synapseclient.core.models.SynapseError("You are not authorized to access fileHandleId %s associated with the Synapse"
    ...
    def _downloadFileHandle(self, fileHandleId, objectId, objectType, destination, retries=5):
        ...
        while retries > 0:
            try:
                ...
                if fileHandle['concreteType'] == synapseclient.core.constants.EXTERNAL_OBJECT_STORE_FILE_HANDLE:
                    profile = self._get_client_authenticated_s3_profile(fileHandle['endpointUrl'], fileHandle['bucket'])
                    downloaded_path = synapseclient.core.remote_file_storage_wrappers.S3ClientWrapper.download_file(
                        fileHandle['bucket'], fileHandle['endpointUrl'], fileHandle['fileKey'], destination,
    ...
    def _download_from_URL(self, url, destination, fileHandleId=None, expected_md5=None):
        ...
        while redirect_count < REDIRECT_LIMIT:
            ...
            if scheme == 'file':
                ...
                destination = synapseclient.core.utils.file_url_to_path(url, verify_exists=True)
                if destination is None:
                    raise IOError("Local file (%s) does not exist." % url)
                break
            elif scheme == 'sftp':
                username, password = self._getUserCredentials(url)
                destination = synapseclient.core.remote_file_storage_wrappers.SFTPWrapper.download_file(
                ...
                temp_destination = synapseclient.core.utils.temp_download_filename(destination, fileHandleId)
                range_header = {"Range": "bytes={start}-".format(start=os.path.getsize(temp_destination))} \
                    if os.path.exists(temp_destination) else {}
                response = synapseclient.core.retry._with_retry(
                ...
                try:
                    synapseclient.core.models.exceptions._raise_for_status(response, verbose=self.debug)
                except synapseclient.core.models.SynapseHTTPError as err:
                    if err.response.status_code == 404:
                        raise synapseclient.core.models.SynapseError("Could not download the file at %s" % url)
                ...
                if response.status_code in [301, 302, 303, 307, 308]:
                    ...
                    if os.path.isdir(destination):
                        filename = synapseclient.core.utils.extract_filename(
                            content_disposition_header=response.headers.get('content-disposition', None),
                            default_filename=synapseclient.core.utils.guess_file_name(url))
                    ...
                    if response.status_code == 206:
                        ...
                        sig = synapseclient.core.utils.md5_for_file(temp_destination)
                    else:
                        mode = 'wb'
                    ...
                    try:
                        with open(temp_destination, mode) as fd:
                            ...
                            for nChunks, chunk in enumerate(response.iter_content(FILE_BUFFER_SIZE)):
                                ...
                                synapseclient.core.utils.printTransferProgress(transferred, toBeTransferred, 'Downloading ',
            ...
            raise synapseclient.core.models.SynapseHTTPError('Too many redirects')

        if actual_md5 is None:  # if md5 not set (should be the case for all except http download)
            actual_md5 = synapseclient.core.utils.md5_for_file(destination).hexdigest()

        # check md5 if given
        if expected_md5 and actual_md5 != expected_md5:
            if delete_on_md5_mismatch and os.path.exists(destination):
                os.remove(destination)
            raise synapseclient.core.models.SynapseMd5MismatchError("Downloaded file {filename}'s md5 {md5} does not match expected MD5 of"
    ...
    def _createExternalFileHandle(self, externalURL, mimetype=None, md5=None, fileSize=None):
        ...
        externalURL = synapseclient.core.utils.as_url(externalURL)
    ...
    def _createExternalObjectStoreFileHandle(self, s3_file_key, file_path, storage_location_id, mimetype=None):
        ...
        file_handle = {'concreteType': 'org.sagebionetworks.repo.model.file.ExternalObjectStoreFileHandle',
                       ...
                       'contentMd5': synapseclient.core.utils.md5_for_file(file_path).hexdigest(),
    ...
    def _getFileHandle(self, fileHandle):
        ...
        uri = "/fileHandle/%s" % (synapseclient.core.utils.id_of(fileHandle),)
    ...
    def _getDefaultUploadDestination(self, parent_entity):
        return self.restGET('/entity/%s/uploadDestination' % synapseclient.core.utils.id_of(parent_entity),
                            endpoint=self.fileHandleEndpoint)
    ...
    def setStorageLocation(self, entity, storage_location_id):
        ...
        if existing_setting is not None:
            ...
            project_destination = {'concreteType':
                                   ...
                                   'settingsType': 'upload',
                                   'locations': locations,
                                   'projectId': synapseclient.core.utils.id_of(entity)
                                   }
    ...
    def getProjectSetting(self, project, setting_type):
        ...
        response = self.restGET('/projectSettings/{projectId}/type/{type}'
                                .format(projectId=synapseclient.core.utils.id_of(project), type=setting_type))
    ...
    def getEvaluation(self, id):
        ...
        evaluation_id = synapseclient.core.utils.id_of(id)
        uri = synapseclient.Evaluation.getURI(evaluation_id)
        return synapseclient.Evaluation(**self.restGET(uri))
    ...
    def getEvaluationByName(self, name):
        ...
        See: :py:mod:`synapseclient.evaluation`
        """
        uri = synapseclient.Evaluation.getByNameURI(urllib.parse.quote(name))
        return synapseclient.Evaluation(**self.restGET(uri))
    ...
    def getEvaluationByContentSource(self, entity):
        ...
        entityId = synapseclient.core.utils.id_of(entity)
        url = "/entity/%s/evaluation" % entityId

        for result in self._GET_paginated(url):
            yield synapseclient.Evaluation(**result)
    ...
    def _findTeam(self, name):
        ...
        for result in self._GET_paginated("/teams?fragment=%s" % name):
            yield synapseclient.Team(**result)
    ...
    def getTeam(self, id):
        ...
        try:
            ...
            if isinstance(id, str):
                ...
                raise ValueError("Can't find team \"{}\"".format(id))
        return synapseclient.Team(**self.restGET('/team/%s' % id))
    ...
    def getTeamMembers(self, team):
        ...
        """
        for result in self._GET_paginated('/teamMembers/{id}'.format(id=synapseclient.core.utils.id_of(team))):
            yield synapseclient.TeamMember(**result)
    ...
    def submit(self, evaluation, entity, name=None, team=None, silent=False, submitterAlias=None, teamName=None):
        ...
        synapseclient.core.utils.require_param(evaluation, "evaluation")
        synapseclient.core.utils.require_param(entity, "entity")

        evaluation_id = synapseclient.core.utils.id_of(evaluation)
        ...
        if team:
            ...
            team_id = synapseclient.core.utils.id_of(team)
        ...
        return synapseclient.Submission(**submitted)

    def _submit(self, submission, entity_etag, eligibility_hash):
        synapseclient.core.utils.require_param(submission, "submission")
        synapseclient.core.utils.require_param(entity_etag, "entity_etag")
    ...
    def _get_contributors(self, evaluation_id, team):
        ...
        if not eligibility['teamEligibility']['isEligible']:
            ...
            if not eligibility['teamEligibility']['isRegistered']:
                raise synapseclient.core.models.SynapseError('Team "{team}" is not registered.'.format(team=team.name))
            if eligibility['teamEligibility']['isQuotaFilled']:
                raise synapseclient.core.models.SynapseError('Team "{team}" has already submitted the full quota of submissions.'
                                   .format(team=team.name))
            raise synapseclient.core.models.SynapseError('Team "{team}" is not eligible.'.format(team=team.name))
    ...
    def _allowParticipation(self, evaluation, user, rights=["READ", "PARTICIPATE", "SUBMIT", "UPDATE_SUBMISSION"]):
        ...
        try:
            ...
            try:
                ...
                if err.response.status_code == 404:
                    raise synapseclient.core.models.SynapseError("The user (%s) does not exist" % str(userId))
                raise
        ...
        if not isinstance(evaluation, synapseclient.Evaluation):
            evaluation = self.getEvaluation(synapseclient.core.utils.id_of(evaluation))
    ...
    def getSubmissions(self, evaluation, status=None, myOwn=False, limit=20, offset=0):
        ...
        for result in self._GET_paginated(uri, limit=limit, offset=offset):
            yield synapseclient.Submission(**result)
    ...
    def getSubmissionBundles(self, evaluation, status=None, myOwn=False, limit=20, offset=0):
        ...
        for bundle in self._getSubmissionBundles(evaluation, status=status, myOwn=myOwn, limit=limit, offset=offset):
            yield (synapseclient.Submission(**bundle['submission']), synapseclient.SubmissionStatus(**bundle['submissionStatus']))
    ...
    def _GET_paginated(self, uri, limit=20, offset=0):
        ...
        while prev_num_results > 0:
            uri = synapseclient.core.utils._limit_and_offset(uri, limit=limit, offset=offset)
    ...
    def getSubmission(self, id, **kwargs):
        ...
        submission_id = synapseclient.core.utils.id_of(id)
        uri = synapseclient.Submission.getURI(submission_id)
        submission = synapseclient.Submission(**self.restGET(uri))
    ...
    def getSubmissionStatus(self, submission):
        ...
        submission_id = synapseclient.core.utils.id_of(submission)
        uri = synapseclient.SubmissionStatus.getURI(submission_id)
        val = self.restGET(uri)
        return synapseclient.SubmissionStatus(**val)
    ...
    def getWiki(self, owner, subpageId=None, version=None):
        ...
        """
        uri = "/entity/{ownerId}/wiki2".format(ownerId=synapseclient.core.utils.id_of(owner))
    ...
    def getWikiHeaders(self, owner):
        ...
        uri = '/entity/%s/wikiheadertree' % synapseclient.core.utils.id_of(owner)
        return [synapseclient.core.models.DictObject(**header) for header in self._GET_paginated(uri)]
    ...
    def _storeWiki(self, wiki, createOrUpdate):  # type: (Wiki, bool) -> Wiki
        ...
        if 'id' in wiki:
            updated_wiki = synapseclient.Wiki(owner=wiki.ownerId, **self.restPUT(wiki.putURI(), wiki.json()))

        # Perform a create if the Wiki has no ID
        else:
            try:
                updated_wiki = synapseclient.Wiki(owner=wiki.ownerId, **self.restPOST(wiki.postURI(), wiki.json()))
            except synapseclient.core.models.SynapseHTTPError as err:
                ...
                if createOrUpdate and ((err.response.status_code == 400 and "DuplicateKeyException" in err.message)
                    ...
                    updated_wiki = synapseclient.Wiki(owner=wiki.ownerId, **self.restPUT(existing_wiki.putURI(),
                                                                           existing_wiki.json()))
                else:
                    raise
        return updated_wiki
    ...
    def _waitForAsync(self, uri, request, endpoint=None):
        ...
        while time.time()-start_time < self.table_query_timeout:
            ...
            if result.get('jobState', None) == 'PROCESSING':
                ...
                if message != '':
                    synapseclient.core.utils.printTransferProgress(progress, total, message, isBytes=False)
                ...
                synapseclient.core.dozer.doze(sleep)
            else:
                break
        else:
            raise synapseclient.core.models.SynapseTimeoutError(
                'Timeout waiting for query results: %0.1f seconds ' % (time.time()-start_time))
        if result.get('jobState', None) == 'FAILED':
            raise synapseclient.core.models.SynapseError(
                result.get('errorMessage', None) + '\n' + result.get('errorDetails', None), asynchronousJobStatus=result)
        if progressed:
            synapseclient.core.utils.printTransferProgress(total, total, message, isBytes=False)
        return result
    ...
    def getColumn(self, id):
        ...
        """
        return synapseclient.Column(**self.restGET(synapseclient.Column.getURI(id)))
    ...
    def getColumns(self, x, limit=100, offset=0):
        ...
        if x is None:
            uri = '/column'
            for result in self._GET_paginated(uri, limit=limit, offset=offset):
                yield synapseclient.Column(**result)
    ...
    def getTableColumns(self, table):
        ...
        """
        uri = '/entity/{id}/column'.format(id=synapseclient.core.utils.id_of(table))
    ...
    def _queryTable(self, query, limit=None, offset=None, isConsistent=True, partMask=None):
        ...
        uri = '/entity/{id}/table/query/async'.format(id=synapseclient.core.utils._extract_synapse_id_from_query(query))
    ...
    def _uploadCsv(self, filepath, schema, updateEtag=None, quoteCharacter='"', escapeCharacter="\\",
        ...
        uploadRequest = {
            ...
            "tableId": synapseclient.core.utils.id_of(schema),
    ...
    def _POST_table_transaction(self, schema, transactionRequests):
        request = {'concreteType': 'org.sagebionetworks.repo.model.table.TableUpdateTransactionRequest',
                   'entityId': synapseclient.core.utils.id_of(schema),
                   'changes': transactionRequests if isinstance(transactionRequests, list) else [transactionRequests]}

        uri = "/entity/{id}/table/transaction/async".format(id=synapseclient.core.utils.id_of(schema))
    ...
    def _check_table_transaction_response(self, response):
        for result in response['results']:
            ...
            if result_type in {synapseclient.core.constants.ROW_REFERENCE_SET_RESULTS,
                               synapseclient.core.constants.TABLE_SCHEMA_CHANGE_RESPONSE,
                               synapseclient.core.constants.UPLOAD_TO_TABLE_RESULT}:
                # if these fail, it we would have gotten an HttpError before the results came back
                pass
            elif result_type == synapseclient.core.constants.ENTITY_UPDATE_RESULTS:
                ...
                if failed_updates:
                    raise synapseclient.core.models.SynapseError(
                        "Not all of the entities were updated. Successful updates: %s.  Failed updates: %s"
    ...
    def _queryTableCsv(self, query, quoteCharacter='"', escapeCharacter="\\", lineEnd=os.linesep, separator=",",
        ...
        uri = "/entity/{id}/table/download/csv/async"\
            .format(id=synapseclient.core.utils._extract_synapse_id_from_query(query))
        ...
        if cached_file_path is not None:
            ...
            if not os.path.exists(cache_dir):
                os.makedirs(cache_dir)
            path = self._downloadFileHandle(file_handle_id,
                                            synapseclient.core.utils._extract_synapse_id_from_query(query),
    ...
    def createColumn(self, name, columnType, maximumSize=None, defaultValue=None, enumValues=None):
        columnModel = synapseclient.Column(name=name, columnType=columnType, maximumSize=maximumSize, defaultValue=defaultValue,
                             enumValue=enumValues)
        return synapseclient.Column(**self.restPOST('/column', json.dumps(columnModel)))
    ...
    def createColumns(self, columns):
        ...
        return [synapseclient.Column(**col) for col in response['list']]
    ...
    def downloadTableFile(self, table, column, downloadLocation=None, rowId=None, versionNumber=None,
        ...
        if isinstance(column, str):
            ...
            if column is None:
                raise synapseclient.core.models.SynapseError("Can't find column \"%s\"." % column)
            column_id = column.id
        elif isinstance(column, synapseclient.Column):
            column_id = column.id
        ...
        if len(result['rows']) == 0 or len(result['rows'][0]['list']) != 1:
            raise synapseclient.core.models.SynapseError(
                'Couldn\'t get file handle for tableId={id}, column={columnId}, row={rowId}, version={versionNumber}'
                    .format(id=table_id, columnId=column_id, rowId=rowId, versionNumber=versionNumber))
                ...
                with zipfile.ZipFile(zipfilepath) as zf:
                    ...
                    for summary in response['fileSummary']:
                        if summary['status'] == 'SUCCESS':
                            ...
                            filepath = synapseclient.core.utils._extract_zip_file_to_directory(zf,
                                                                                               summary['zipEntryName'],
                                                                                               cache_dir)
    ...
    def _build_table_download_file_handle_list(self, table, columns):
        ...
        for row in table:
            for col_index in col_indices:
                ...
                if synapseclient.core.utils._is_integer(file_handle_id):
    ...
    def _get_default_entity_view_columns(self, view_type_mask):
        return [synapseclient.Column(**col)
    ...
    def _get_annotation_entity_view_columns(self, scope_ids, view_type_mask):
        ...
        while True:
            ...
            columns.extend(synapseclient.Column(**column) for column in response['results'])
    ...
    def _getEntity(self, entity, version=None):
        ...
        uri = '/entity/'+synapseclient.core.utils.id_of(entity)
        if version:
    ...
    def _createEntity(self, entity):
        ...
        return self.restPOST(uri='/entity', body=json.dumps(synapseclient.core.utils.get_properties(entity)))
    ...
    def _updateEntity(self, entity, incrementVersion=True, versionLabel=None):
        ...
        uri = '/entity/%s' % synapseclient.core.utils.id_of(entity)

        if synapseclient.entity.is_versionable(entity):
        ...
        return self.restPUT(uri, body=json.dumps(synapseclient.core.utils.get_properties(entity)))
    ...
    def findEntityId(self, name, parent=None):
        ...
        entity_lookup_request = {"parentId": synapseclient.core.utils.id_of(parent) if parent else None,
                                 "entityName": name}
        try:
            return self.restPOST("/entity/child", body=json.dumps(entity_lookup_request)).get("id")
        except synapseclient.core.models.SynapseHTTPError as e:
    ...
    def sendMessage(self, userIds, messageSubject, messageBody, contentType="text/plain"):
        ...
        fileHandleId = synapseclient.core.upload.multipart_upload_string(self, messageBody, contentType)
        message = dict(
            recipients=userIds,
    ...
    def _generateSignedHeaders(self, url, headers=None):
        ...
        if self.credentials is None:
            raise synapseclient.core.models.SynapseAuthenticationError("Please login")
    ...
    def restPOST(self, uri, body, endpoint=None, headers=None, retryPolicy={}, **kwargs):
        ...
        response = synapseclient.core.retry._with_retry(
            lambda: self._requests_session.post(uri, data=body, headers=headers, **kwargs),
    ...
    def restPUT(self, uri, body=None, endpoint=None, headers=None, retryPolicy={}, **kwargs):
        ...
        response = synapseclient.core.retry._with_retry(
            lambda: self._requests_session.put(uri, data=body, headers=headers, **kwargs),
            verbose=self.debug, **retryPolicy)
        synapseclient.core.models.exceptions._raise_for_status(response, verbose=self.debug)
        return self._return_rest_body(response)
