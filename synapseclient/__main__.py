...
def build_parser():
    ...
    description_group_store.add_argument(
        "--description",
        metavar="DESCRIPTION",
        type=str,
    ...
    parser_store.add_argument(
        "FILE", nargs="?", type=str, help="file to be added to synapse."
    )
    parser_store.set_defaults(func=store)

    parser_add = subparsers.add_parser(
        "add",  # Python 3.2+ would support alias=['store']
        help="uploads and adds a file to Synapse",
    )
    ...
    description_group_add.add_argument(
        "--description",
        metavar="DESCRIPTION",
        type=str,
        help="Description of data object in Synapse.",
    ...
    parser_cp.add_argument(
        "id", metavar="syn123", type=str, help="Id of entity in Synapse to be copied."
    )
    parser_cp.add_argument(
        "--destinationId",
        metavar="syn123",
    ...
    parser_cp.add_argument(
        ...
        default=None,
        help=(
            "Synapse version number of File or Link to retrieve. "
    ...
    parser_cp.add_argument(
        ...
        default="traceback",
        help=(
            "Has three values to set the provenance of the copied entity-"
    ...
    parser_get_dl_list = subparsers.add_parser(
        "get-download-list", help="Download files from the Synapse download cart"
    )
    parser_get_dl_list.add_argument(
        "--downloadLocation",
        metavar="path",
        type=str,
        default="./",
    ...
    parser_associate = subparsers.add_parser(
        "associate",
        help=(
    ...
    parser_associate.add_argument(
        "path", metavar="path", type=str, help="local file path"
    )
    parser_associate.add_argument(
        "--limitSearch",
        metavar="projId",
    ...
    parser_delete = subparsers.add_parser(
        "delete", help="removes a dataset from Synapse"
    )
    ...
    parser_delete.add_argument(
        "--version", type=str, help="Version number to delete of given entity."
    )
    parser_delete.set_defaults(func=delete)

    parser_query = subparsers.add_parser(
        "query", help="Performs SQL like queries on Synapse"
    )
...
See https://docs.synapse.org/rest/org/sagebionetworks/repo/web/controller/TableExamples.html' for more information""",
    ...
    parser_submit = subparsers.add_parser(
        "submit", help="submit an entity or a file for evaluation"
    )
    ...
    parser_submit.add_argument(
        "--evaluation", type=str, help=argparse.SUPPRESS
    )  # mainly to maintain the backward compatibility
    parser_submit.add_argument(
        "--entity",
        "--eid",
        "--entityId",
    ...
    parser_submit.add_argument(
        "--file", "-f", type=str, help="File to be submitted to the challenge"
    )
    parser_submit.add_argument(
        "--parentId",
        "--parentid",
    ...
    parser_submit.add_argument(
        "--teamName", "--team", type=str, help="Submit of behalf of a registered team"
    )
    parser_submit.add_argument(
        "--submitterAlias",
        "--alias",
    ...
    parser_list.add_argument(
        "id", metavar="syn123", type=str, help="Synapse ID of a project or folder"
    )
    parser_list.add_argument(
        "-r",
        "--recursive",
    ...
    parser_set_annotations.add_argument(
        "--id",
        metavar="syn123",
        type=str,
        required=True,
    ...
    parser_get_annotations = subparsers.add_parser(
        "get-annotations", help="show annotations records"
    )
    parser_get_annotations.add_argument(
        "--id",
        metavar="syn123",
        type=str,
        required=True,
    ...
    parser_create = subparsers.add_parser(
        "create", help="Creates folders or projects on Synapse"
    )
    ...
    description_group_create.add_argument(
        "--description",
        metavar="DESCRIPTION",
        type=str,
    ...
    parser_store_table = subparsers.add_parser(
        "store-table", help="Creates a Synapse Table given a csv"
    )
    parser_store_table.add_argument(
        "--name", metavar="NAME", type=str, required=True, help="Name of Table"
    )
    parser_store_table.add_argument(
        "--parentid",
        "--parentId",
    ...
    parser_store_table.add_argument(
        "--csv", metavar="foo.csv", type=str, required=False, help="Path to csv"
    )
    parser_store_table.set_defaults(func=storeTable)

    parser_onweb = subparsers.add_parser(
        "onweb", help="opens Synapse website for Entity"
    )
    ...
    parser_test_encoding = subparsers.add_parser(
        "test-encoding", help="test character encoding to help diagnose problems"
    )
    ...
    parser_get_sts_token.add_argument(
        "permission", type=str, choices=["read_write", "read_only"]
    )
    parser_get_sts_token.add_argument(
        "-o",
        "--output",
    ...
    parser_migrate = subparsers.add_parser(
        "migrate", help="Migrate Synapse entities to a different storage location"
    )
    parser_migrate.add_argument("id", type=str, help="Synapse id")
    parser_migrate.add_argument(
        "dest_storage_location_id",
        type=str,
        help="Destination Synapse storage location id",
    )
    parser_migrate.add_argument(
        "db_path",
        type=str,
...
def _prompt_for_credentials(user=None):
    ...
    secret_prompt = (
        f"Api key, or auth token for user {user}:" if user else "Auth token:"
    )
